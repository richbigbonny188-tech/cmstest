...
class GMIPayment_ORIGIN {
	...
	public function log($message) {
	    ...
	    if ($logger === null) {
            ...
            $logger = $loggerBuilder->omitRequestData()->changeNamespace('payment.ipayment')->build();
        }
	...
	protected function _loadPaymentModuleConfiguration() {
		...
		$config_query = "SELECT `value` as `configuration_value` FROM gx_configurations WHERE `key` = ':config_key'";
		foreach($keys as $key) {
			...
			while($key_row = xtc_db_fetch_array($key_result)) {
				$key = str_replace('configuration/MODULE_PAYMENT_'.strtoupper($this->_code).'_', '', $key);
	...
	protected function _getBasicFormData() {
        $gx_version = require DIR_FS_CATALOG . 'release_info.php';
		$formdata = array(
	...
	public function getFormData($orders_id, $order) {
		...
		$formdata = array_merge($formdata, array(
			...
			'addr_country' => $this->_findISO2byCountry($order->customer['country']),
		));
	...
	protected function _findISO2byCountry($country) {
		$query = "SELECT countries_iso_code_2 FROM countries WHERE countries_name = '".xtc_db_input($country)."'";
		$iso = false;
		$result = xtc_db_query($query);
		while($row = xtc_db_fetch_array($result)) {
			$iso = $row['countries_iso_code_2'];
		}
		return $iso;
	}
}
