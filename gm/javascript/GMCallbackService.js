/* GMCallbackService.js <?php
 #   --------------------------------------------------------------
 #   GMCallbackService.js 2018-06-15
 #   Gambio GmbH
 #   http://www.gambio.de
...
/*<?php
 ...
 {
 ?>*/
var gm_callback_service=new GMCallbackService();function GMCallbackService(){this.check_callback=function(){$('#gm_callback_service_error_message').load('request_port.php?module=CallbackService&action=check&'+$('#gm_callback_service_form').serialize(),function(errors){if(errors=='')gm_callback_service.send_form();else{$('#gm_callback_service_error_message').html(errors);try{Recaptcha.reload()}catch(e){$('#vvcode').val('');$('#vvcode_image').attr('src','request_port.php?rand='+Math.random()+'&module=CreateVVCode')}}})};this.send_form=function(){jQuery.ajax({data:$('#gm_callback_service_form').serialize(),url:'request_port.php?module=CallbackService&action=send',type:"POST",success:function(sent_success){$("#gm_callback_service_success").html(sent_success)}})};}$(document).ready(function(){$("#gm_callback_service_send").click(function(){gm_callback_service.check_callback();return false})});
/*<?php
 }
 else
 {
		...
		$('#gm_callback_service_error_message').load('request_port.php?module=CallbackService&action=check'
			...
			function(errors){
				...
				else
				{
					...
					{
						$('#vvcode').val('');
						$('#vvcode_image').attr('src', 'request_port.php?rand=' + Math.random() + '&module=CreateVVCode');
					}
				}
			}
		);
