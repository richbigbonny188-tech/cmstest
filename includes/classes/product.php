...
class product_ORIGIN implements ProductDataInterface
{
	...
	function buildDataArray(&$array, $image = 'thumbnail')
	{
        ...
        $min_order_qty = $this->gm_min_order($array['products_id']);
        $products_price = $xtPrice->xtcGetPrice($array['products_id'], true, $min_order_qty, $array['products_tax_class_id'], $array['products_price'], 1, 0, true, true, $t_combi['products_properties_combis_id'] ?? false, true);
		...
		if($_SESSION['customers_status']['customers_status_show_price'] != '0' && $xtPrice->gm_check_price_status($array['products_id']) == 0)
		{
            ...
            // todo implment logic to product listing
			if($_SESSION['customers_status']['customers_fsk18_purchasable'] === '0')
			{
				if($array['products_fsk18'] == '0')
				{
					...
					$gm_qty = xtc_draw_input_field('products_qty', $min_order_qty, 'size="3" id="gm_attr_calc_qty_' . $array['products_id'] . '" onkeyup="gm_calc_prices_listing(\'' . $array['products_id'] . '\')"', 'text', true, "gm_listing_form gm_class_input");
