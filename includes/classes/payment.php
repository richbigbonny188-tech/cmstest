...
class payment_ORIGIN
{
	...
	public function __construct($module = '')
	{
		...
		if(defined('MODULE_PAYMENT_INSTALLED') && xtc_not_null(MODULE_PAYMENT_INSTALLED))
		{
			...
			if(isset($GLOBALS['order']->customer['payment_unallowed'])) {
                $unallowed_modules = array_merge($unallowed_modules, explode(',', $GLOBALS['order']->customer['payment_unallowed']));
            }
			...
			for($i = 0, $n = sizeof($include_modules); $i < $n; $i++)
			{
				if(!in_array($include_modules[$i]['class'], $unallowed_modules))
				{
                    ...
                    $allowedConstantName = 'MODULE_PAYMENT_' . strtoupper(str_replace('.php', '', $include_modules[$i]['file'])) . '_ALLOWED';
					if(defined($allowedConstantName) && constant($allowedConstantName) !== '')
					{
						$unallowed_zones = explode(',', constant($allowedConstantName));
					}
					else
					{
	...
	{
		if(is_array($this->modules) && is_object($GLOBALS[$this->selected_module] ?? null) && method_exists($GLOBALS[$this->selected_module], 'update_status'))
		{
	...
	function refresh()
	{
		if(is_array($this->modules) && is_object($GLOBALS[$this->selected_module] ?? null) && method_exists($GLOBALS[$this->selected_module], 'refresh'))
		{
		...
		if(is_array($this->modules))
		{
			...
			foreach($this->modules as $value)
			{
				...
				if(!empty($GLOBALS[$class]->enabled))
				{
					...
					{
						$selection = $this->extendSelection($selection);
	...
	public function getPaymentModuleSelectionLogoUrl($selection)
	{
		...
		foreach($languageExtensions as $languageExtension)
		{
			...
			{
				$logoFileCandidate = $selection['id'] . $languageExtension . $extension;
		...
		$logoUrl = xtc_href_link('images/icons/payment/' . $logoFile, '', 'SSL', false, false, false, true, true);
		return $logoUrl;
	}

	protected function extendSelection($selection)
	{
		$selectionDefaults = [
			'logo_url' => $this->getPaymentModuleSelectionLogoUrl($selection),
			'logo_alt' => $selection['module'],
		];
		$selection = array_merge($selectionDefaults, $selection);
		return $selection;
	}
	...
	function pre_confirmation_check()
	{
		...
		if(is_array($this->modules))
		{
			if(is_object($GLOBALS[$this->selected_module] ?? null) && $GLOBALS[$this->selected_module]->enabled)
			{
				if(!empty($GLOBALS['credit_covers']))
				{ // GV Code ICW CREDIT CLASS Gift Voucher System
