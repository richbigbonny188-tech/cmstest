...
class zonese_ORIGIN
{
    ...
    {
        $this->code        = 'zonese';
        $this->title       = defined('MODULE_SHIPPING_ZONESE_TEXT_TITLE') ? MODULE_SHIPPING_ZONESE_TEXT_TITLE : '';
        $this->description = defined('MODULE_SHIPPING_ZONESE_TEXT_DESCRIPTION') ? MODULE_SHIPPING_ZONESE_TEXT_DESCRIPTION : '';
        $this->sort_order  = defined('MODULE_SHIPPING_ZONESE_SORT_ORDER') ? MODULE_SHIPPING_ZONESE_SORT_ORDER : '0';
        $this->icon        = '';
        $this->tax_class   = defined('MODULE_SHIPPING_ZONESE_TAX_CLASS') ? MODULE_SHIPPING_ZONESE_TAX_CLASS : '0';
        ...
        if ($dest_zone == 0) {
            ...
            for ($i = 0; $i < $size; $i += 2) {
                if ($shipping_weight <= $zones_table[$i]) {
                    $shipping        = $zones_table[$i + 1] ?? -1;
                    $shipping_method = MODULE_SHIPPING_ZONESE_TEXT_WAY . ' ' . $dest_country . ' : ('
                                       . $shipping_num_boxes . ' x ' . $shipping_weight . ' '
                                       . MODULE_SHIPPING_ZONESE_TEXT_UNITS . ')';
                    break;
                }
            }
            ...
            if ($shipping == -1) {
                ...
                $shipping_cost = (($shipping * $shipping_num_boxes)
                                  + ((double)constant('MODULE_SHIPPING_ZONESE_HANDLING_' . $dest_zone)
                                     * $shipping_num_boxes));
            }
        }
    ...
    function check()
    {
        if (!isset($this->_check)) {
            $check_query  = xtc_db_query("SELECT `value` from `gx_configurations` where `key` = 'configuration/MODULE_SHIPPING_ZONESE_STATUS'");
    ...
    function install()
    {
        ...
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_SHIPPING_ZONESE_STATUS', 'True', '0', 'switcher', now())");
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_ZONESE_ALLOWED', '', '0', now())");
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_SHIPPING_ZONESE_TAX_CLASS', '0', '0', 'tax-class', now())");
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_ZONESE_SORT_ORDER', '0', '0', now())");
        for ($i = 1; $i <= $this->num_zones; $i++) {
            $default_countries = '';
            if ($i == 1) {
                $default_countries = 'DE,AT,CH';
            }
            xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_ZONESE_COUNTRIES_"
                         . $i . "', '" . $default_countries . "', '0', now())");
            xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_ZONESE_COST_"
                         . $i . "', '', '0', now())");
            xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_ZONESE_HANDLING_"
                         . $i . "', '0', '0', now())");
        }
    }
    ...
    {
        $keys = [
            'configuration/MODULE_SHIPPING_ZONESE_STATUS',
            'configuration/MODULE_SHIPPING_ZONESE_ALLOWED',
            'configuration/MODULE_SHIPPING_ZONESE_TAX_CLASS',
            'configuration/MODULE_SHIPPING_ZONESE_SORT_ORDER'
        ];
        
        for ($i = 1; $i <= $this->num_zones; $i++) {
            $keys[] = 'configuration/MODULE_SHIPPING_ZONESE_COUNTRIES_' . $i;
            $keys[] = 'configuration/MODULE_SHIPPING_ZONESE_COST_' . $i;
            $keys[] = 'configuration/MODULE_SHIPPING_ZONESE_HANDLING_' . $i;
        }
