...
class b2czones_ORIGIN
{
    ...
    {
        $this->code        = 'b2czones';
        $this->title       = defined('MODULE_SHIPPING_B2CZONES_TEXT_TITLE') ? MODULE_SHIPPING_B2CZONES_TEXT_TITLE : '';
        $this->description = defined('MODULE_SHIPPING_B2CZONES_TEXT_DESCRIPTION') ? MODULE_SHIPPING_B2CZONES_TEXT_DESCRIPTION : '';
        $this->sort_order  = defined('MODULE_SHIPPING_B2CZONES_SORT_ORDER') ? MODULE_SHIPPING_B2CZONES_SORT_ORDER : '0';
        $this->icon        = '';
        $this->tax_class   = defined('MODULE_SHIPPING_B2CZONES_TAX_CLASS') ? MODULE_SHIPPING_B2CZONES_TAX_CLASS : '0';
    ...
    function quote($method = '')
    {
        ...
        for ($i = 1; $i <= $this->num_zones; $i++) {
            $countries_table = constant('MODULE_SHIPPING_B2CZONES_COUNTRIES_' . $i);
        ...
        if ($dest_zone == 0) {
            ...
            $shipping   = -1;
            $zones_cost = constant('MODULE_SHIPPING_B2CZONES_COST_' . $dest_zone);
            ...
            for ($i = 0; $i < $size; $i += 2) {
                if ($shipping_weight <= $zones_table[$i]) {
                    ...
                    $shipping_method = MODULE_SHIPPING_B2CZONES_TEXT_WAY . ' ' . $dest_country . ' : ('
                                       . $shipping_num_boxes . ' x ' . $shipping_weight . ' '
                                       . MODULE_SHIPPING_B2CZONES_TEXT_UNITS . ')';
                    break;
                }
            }
            ...
            if ($shipping == -1) {
                $error           = true;
                $shipping_cost   = 0;
                $shipping_method = MODULE_SHIPPING_B2CZONES_UNDEFINED_RATE;
            } else {
                $shipping_cost = (($shipping * $shipping_num_boxes)
                                  + ((double)constant('MODULE_SHIPPING_B2CZONES_HANDLING_' . $dest_zone)
                                     * $shipping_num_boxes));
            }
        }
        ...
        $this->quotes = [
            'id'      => $this->code,
            'module'  => MODULE_SHIPPING_B2CZONES_TEXT_TITLE,
            'methods' => [
                [
                    'id'    => $this->code,
        ...
        if ($error == true) {
            $this->quotes['error'] = MODULE_SHIPPING_B2CZONES_UNDEFINED_RATE;
        }
    ...
    function check()
    {
        if (!isset($this->_check)) {
            $check_query  = xtc_db_query("SELECT `value` from `gx_configurations` where `key` = 'configuration/MODULE_SHIPPING_B2CZONES_STATUS'");
    ...
    function install()
    {
        ...
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_SHIPPING_B2CZONES_STATUS', 'True', '0', 'switcher', now())");
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_B2CZONES_ALLOWED', '', '0', now())");
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_SHIPPING_B2CZONES_TAX_CLASS', '0', '0', 'tax-class', now())");
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_B2CZONES_SORT_ORDER', '0', '0', now())");
        for ($i = 1; $i <= $this->num_zones; $i++) {
            $default_countries = '';
            if ($i == 1) {
                $default_countries = 'DE,CH,AT';
            }
            xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_B2CZONES_COUNTRIES_"
                         . $i . "', '" . $default_countries . "', '0', now())");
            xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_B2CZONES_COST_"
                         . $i . "', '', '0', now())");
            xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_B2CZONES_HANDLING_"
                         . $i . "', '0', '0', now())");
        }
    }
    ...
    {
        $keys = [
            'configuration/MODULE_SHIPPING_B2CZONES_STATUS',
            'configuration/MODULE_SHIPPING_B2CZONES_ALLOWED',
            'configuration/MODULE_SHIPPING_B2CZONES_TAX_CLASS',
            'configuration/MODULE_SHIPPING_B2CZONES_SORT_ORDER'
        ];
        
        for ($i = 1; $i <= $this->num_zones; $i++) {
            $keys[] = 'configuration/MODULE_SHIPPING_B2CZONES_COUNTRIES_' . $i;
            $keys[] = 'configuration/MODULE_SHIPPING_B2CZONES_COST_' . $i;
            $keys[] = 'configuration/MODULE_SHIPPING_B2CZONES_HANDLING_' . $i;
        }
...
MainFactory::load_origin_class('b2czones');
