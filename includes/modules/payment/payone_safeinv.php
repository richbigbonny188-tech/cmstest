...
class payone_safeinv_ORIGIN extends payone_master
{
    ...
    protected function _pyvEnabled()
    {
        ...
        $pyv_enabled = $pyv_enabled
                       && ($_SESSION['customer_b2b_status'] == 0
                           || $this->pg_config['genre_specific']['payolution_b2b_enabled'] == 'true');
    ...
    function _paymentDataForm($active_genre_identifier)
    {
        ...
        $agreementLink   = sprintf('<a class="js-open-modal" href="%s" data-modal-type="iframe" target="_blank">%s</a>',
                                   xtc_href_link('shop.php',
                                                 'do=ExtraContent/PayolutionNote&config='
                                                 . $this->_getActiveGenreIdentifier(),
                                                 'SSL'),
        ...
        include DIR_FS_CATALOG . 'themes/Honeygrid/html/system/checkout_payment_payone_safeinv_form.html';
    ...
    function pre_confirmation_check()
    {
        ...
        if (isset($_POST['p1-safeinv-doby']) && isset($_POST['p1-safeinv-dobm']) && isset($_POST['p1-safeinv-dobd'])) {
            $dobDateTime            = new DateTime(sprintf('%04d-%02d-%02d',
                                                           $_POST['p1-safeinv-doby'],
                                                           $_POST['p1-safeinv-dobm'],
                                                           $_POST['p1-safeinv-dobd']));
    ...
    function payment_action()
    {
        ...
        if ($response instanceof Payone_Api_Response_Preauthorization_Approved) {
            ...
            $this->_updateOrdersStatus($GLOBALS['insert_id'],
                                       0,
                                       strtolower((string)$response->getStatus()),
    ...
    protected function _storePayolutionPaymentInstruction(
        Payone_Api_Response_Authorization_Abstract $authorization,
        order $order
    ) {
        $due_date   = date('Y-m-d',
                           strtotime('+' . (int)$this->pg_config['genre_specific']['payolution_due_days'] . ' days'));
