...
class hppp_ORIGIN
{
    ...
    function __construct()
    {
        ...
        $this->title_txt            = defined('MODULE_PAYMENT_HPPP_TEXT_TITLE') ? MODULE_PAYMENT_HPPP_TEXT_TITLE : '';
        $this->title                = sprintf(HGW_LOGO, DIR_WS_CATALOG) . $this->title_txt;
        $this->description          = MODULE_PAYMENT_HPPP_TEXT_DESC . '<br/><i>[' . $this->hgw->version . ']</i>'
                                      . $this->hgw->modulConfButton;
        $this->sort_order           = defined('MODULE_PAYMENT_HPPP_SORT_ORDER') ? MODULE_PAYMENT_HPPP_SORT_ORDER : '0';
        ...
        $this->info                 = defined('MODULE_PAYMENT_HPPP_TEXT_INFO') ? MODULE_PAYMENT_HPPP_TEXT_INFO : '';
        $this->tmpOrders            = true;
        $this->hgw->actualPaymethod = strtoupper($this->pm);
        $this->prefix               = 'configuration/MODULE_PAYMENT_HPPP_';
    ...
    function update_status()
    {
        ...
        if (($this->enabled == true) && ((int)MODULE_PAYMENT_HPPP_ZONE > 0)) {
            ...
            $sql        = xtc_db_query("SELECT zone_id FROM " . TABLE_ZONES_TO_GEO_ZONES . " WHERE geo_zone_id = '"
                                       . MODULE_PAYMENT_HPPP_ZONE . "' AND zone_country_id = '"
    ...
    function selection()
    {
        ...
        if ($getConf['transactionMode'] == '1' && isset($order)
            && strpos(strtolower(MODULE_PAYMENT_HPPP_TEST_ACCOUNT),
    ...
    {
        return [
            'configuration/MODULE_PAYMENT_HPPP_STATUS',
            'configuration/MODULE_PAYMENT_HPPP_TEST_ACCOUNT',
            'configuration/MODULE_PAYMENT_HPPP_PROCESSED_STATUS_ID',
            'configuration/MODULE_PAYMENT_HPPP_PENDING_STATUS_ID',
            'configuration/MODULE_PAYMENT_HPPP_CANCELED_STATUS_ID',
            'configuration/MODULE_PAYMENT_HPPP_SORT_ORDER',
            'configuration/MODULE_PAYMENT_HPPP_ALLOWED',
            'configuration/MODULE_PAYMENT_HPPP_ZONE',
        ];
    }
}
