...
if ($_SESSION['customers_status']['customers_status_id'] == '0') {
	...
	if ($SofortLib_TransactionData->isError()) {
		xtc_db_query("UPDATE `gx_configurations` SET `value` = '".MODULE_PAYMENT_SOFORT_KEYTEST_ERROR_DESC."' where `key` = 'configuration/MODULE_PAYMENT_SOFORT_MULTIPAY_AUTH'");
		xtc_db_query("UPDATE `gx_configurations` SET `value` = '' where `key` = 'configuration/MODULE_PAYMENT_SOFORT_MULTIPAY_APIKEY'");
		ob_end_clean();
		echo "f".MODULE_PAYMENT_SOFORT_KEYTEST_ERROR;
	} else {
		xtc_db_query("UPDATE `gx_configurations` SET `value` = '".MODULE_PAYMENT_SOFORT_KEYTEST_SUCCESS_DESC." ".date("d.m.Y")."' where `key` = 'configuration/MODULE_PAYMENT_SOFORT_MULTIPAY_AUTH'");
		xtc_db_query("UPDATE `gx_configurations` SET `value` = '".HelperFunctions::escapeSql($configKey)."' where `key` = 'configuration/MODULE_PAYMENT_SOFORT_MULTIPAY_APIKEY'");
		ob_end_clean();
