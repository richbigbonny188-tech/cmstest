<?php
/* --------------------------------------------------------------
   xtc_php_mail.inc.php 2022-04-11 gm
   Gambio GmbH
   http://www.gambio.de
...
function xtc_php_mail(
    ...
    try {
        ...
        $sender = MainFactory::create('EmailContact',
                                      MainFactory::create('EmailAddress', $from_email_address),
        ...
        if (!empty($reply_email_address)) {
            $replyTo = MainFactory::create('EmailContact',
                                           MainFactory::create('EmailAddress', $reply_email_address),
            ...
            if (is_array($path_to_attachment) && empty($path_to_attachment) === false) {
                foreach ($path_to_attachment as $file) {
                    ...
                    if (file_exists((string)$path)) {
                        if (isset($file['configuration'])) {
                            $hash             = $file['configuration']['hash'];
                            $configurationKey = $file['configuration']['configurationKey'];
                            $languageCode     = $file['configuration']['languageCode'];
                            
                            $attachmentConfiguration = MainFactory::create('AttachmentConfiguration',
                        ...
                        $attachments->add(MainFactory::create('EmailAttachment',
                                                              $attachmentPath,
                                                              $attachmentName,
