<?php
/*------------------------------------------------------------------------------
 ActionUpdateVisibility.php 2020-10-19
 Gambio GmbH
 http://www.gambio.de
...
/**
 * Class ActionUpdateVisibility
 *
 * @package Gambio\Admin\Modules\Dashboard\App\Actions
 * @codeCoverageIgnore
 */
class ActionUpdateVisibility extends AbstractAction
{
    /**
    ...
    public function handle(Request $request, HttpResponse $response): HttpResponse
    {
        if ($request->getAttribute('status') === 'true') {
            $this->setupWizardService->show();
        } else {
            $this->setupWizardService->hide();
        }
