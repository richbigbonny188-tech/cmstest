<?php
/* --------------------------------------------------------------
   OrderMailBuilder.php 2023-07-07
   Gambio GmbH
   http://www.gambio.de
...
class OrderMailBuilder
{
    /**
     * @var string html of the order
     */
    protected $htmlMailBody = '';
    ...
    /**
     * Returns order data for given order ID.
     *
     * @param int $orderId
     *
     * @return array
    ...
    public function getOrderData(int $orderId): array
    {
        ...
        return xtc_db_num_rows($query) === 1 ? xtc_db_fetch_array($query) : [];
    }
    
    
    /**
     * Builds the html and txt body for the given order.
     *
     * @param $orderData
     *
     * @return $this
     */
    public function build($orderData): OrderMailBuilder
    {
        $t_order = MainFactory::create('order', $orderData['orders_id']);
        ...
        $t_order_query = xtc_db_query("SELECT
									  ...
									  WHERE op.orders_id = " . (int)$orderData['orders_id']);
        ...
        while ($t_order_data_values = xtc_db_fetch_array($t_order_query)) {
            $t_attributes_query = xtc_db_query("
												...
												WHERE
													orders_products_id	= '"
                                               . $t_order_data_values['orders_products_id'] . "'
												AND
													orders_id			= '" . (int)$orderData['orders_id'] . "'
			");
            ...
            while ($t_attributes_data_values = xtc_db_fetch_array($t_attributes_query)) {
                $t_attributes_data  .= '<br />' . $t_attributes_data_values['products_options'] . ':'
                                       . $t_attributes_data_values['products_options_values'];
                $t_attributes_model .= '<br />' . xtc_get_attributes_model($t_order_data_values['products_id'],
                                                                           $t_attributes_data_values['products_options_values'],
            ...
            if ($t_order_data_values['products_quantity'] == 0) {
                ...
                $t_products_price_single = xtc_format_price_order($t_order_data_values['final_price']
                                                                  / $t_order_data_values['products_quantity'],
                                                                  1,
        ...
        // GET WITHDRAWAL
        if (GROUP_CHECK == 'true' && (isset($t_order->customer['status']) || isset($t_order->info['status']))) {
            $group_check = "and group_ids LIKE '%c_" . ($t_order->customer['status'] ?? $t_order->info['status']) . "_group%'";
        }
        $t_shop_content_query = xtc_db_query("SELECT
											...
											WHERE content_group='" . (int)gm_get_conf('GM_WITHDRAWAL_CONTENT_ID') . "' "
                                             . ($group_check ?? null) . "
        ...
        $coo_lang_file_master->init_from_lang_file('lang/' . $t_order->info['language'] . '/modules/payment/'
                                                   . $t_order->info['payment_method'] . '.php');
        ...
        switch ($t_order->info['payment_method']) {
            ...
            case 'eustandardtransfer':
                $t_payment_info_html = sprintf(MODULE_PAYMENT_EUTRANSFER_TEXT_DESCRIPTION,
            ...
            // MONEYORDER
            case 'moneyorder':
                $t_payment_info_html = sprintf(MODULE_PAYMENT_MONEYORDER_TEXT_DESCRIPTION,
                                               MODULE_PAYMENT_MONEYORDER_PAYTO,
                                               nl2br(STORE_NAME_ADDRESS));
                $t_payment_info_text = str_replace("<br />",
                                                   "\n",
                                                   sprintf(MODULE_PAYMENT_MONEYORDER_TEXT_DESCRIPTION,
                                                           MODULE_PAYMENT_MONEYORDER_PAYTO,
                                                           nl2br(STORE_NAME_ADDRESS)));
                break;
            default:
        ...
        // GET E-MAIL LOGO
        $t_mail_logo = '';
        $t_logo_mail = MainFactory::create_object('GMLogoManager', ["gm_logo_mail"]);
        ...
        $coo_send_order_content_view->set_('order_id', $orderData['orders_id']);
        ...
        $this->htmlMailBody = $t_mail_content_array['html'];
        
        // GET TXT MAIL CONTENT
        $this->txtMailBody = $t_mail_content_array['txt'];
    ...
    /**
     * get html of the order mail body
     *
     * @return string
     */
    public function getHtmlMailBody(): string
    {
        return $this->htmlMailBody;
    }
    
    
    /**
     * get txt of the order mail body
     *
     * @return string
     */
    public function getTxtMailBody(): string
    {
