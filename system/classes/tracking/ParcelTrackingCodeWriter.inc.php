<?php
/* --------------------------------------------------------------
   ParcelTrackingCodeWriter.inc.php 2020-08-12
   Gambio GmbH
   http://www.gambio.de
...
class ParcelTrackingCodeWriter
{
     ...
     *
     * @param string              $parcelServiceName
     *
     * @return int
     * @throws \Exception
     */
                                      ...
                                      $parcelServiceName = '')
	{
		...
		$languageId        = $this->_getLanguageIdByOrderId($p_orderId);
		$parcelServiceName = empty($parcelServiceName) ? $parcelService->getName() : $parcelServiceName;
		$url               = $p_trackingUrl;
		$comment           = $this->resolveTrackingCodePlaceholder($parcelService->getCommentByLanguageId($languageId),
		                                                           '');
