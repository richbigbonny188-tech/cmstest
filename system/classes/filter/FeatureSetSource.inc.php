<?php
/* --------------------------------------------------------------
   FeatureSetSource.inc.php 2023-04-26
   Gambio GmbH
   http://www.gambio.de
...
{	
	...
	protected function get_where_for_sets( $p_feature_values, &$p_value_conjunctions )
	{
		...
		{
			$t_where_strings[] = $this->build_where_string($t_feature_values, $p_value_conjunctions[$t_feature_id] ?? null);
		}
	...
	protected function get_where_for_features( $p_feature_values, &$p_value_conjunctions )
	{
		...
		foreach ($p_feature_values as $t_skip_feature_id => $t_skip_feature_values)
		{
			...
			foreach ($p_feature_values as $t_feature_id => $t_feature_values)
			{
				if(($p_value_conjunctions[$t_feature_id] ?? null) == 0 && $t_feature_id == $t_skip_feature_id)
				{
					continue;
				}
				$t_where_parts[] = $this->build_where_string($t_feature_values, $p_value_conjunctions[$t_feature_id] ?? null);
			}
    ...
    /**
     * @param $p_category_id
     * @param $p_where_strings
     * @param array $p_features_ids
     * @param bool $p_filter_by_feature
     * @return array
     */
	public function get_values_from_where_strings( $p_category_id, $p_where_strings, $p_features_ids=array(), $p_filter_by_feature=false )
	{
        ...
        if (GROUP_CHECK == 'true') {
            $t_query.= ' AND p.group_permission_'.$this->_get_customer_status_id().'=1';
        }
        if (!$this->_is_customers_fsk18_display_enabled()) {
            $t_query.= ' AND p.products_fsk18 = 0';
        }
    ...
    /**
     * clean the customers_status_id session variable;
     */
    protected function _get_customer_status_id()
    {
        return (int) $_SESSION['customers_status']['customers_status_id'];
    }
    
    
    protected function _is_customers_fsk18_display_enabled()
    {
        return (int)$_SESSION['customers_status']['customers_fsk18_display'];
    }
}
