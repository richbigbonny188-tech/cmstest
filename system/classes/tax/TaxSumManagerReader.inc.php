...
class TaxSumManagerReader
{
	...
	protected function buildTaxSumItemsArray()
	{
		...
		while($dataset = xtc_db_fetch_array($result))
		{
			...
			$taxSumGroup->setSumNetDefaultCurrency($dataset['t_sum_net_default_currency'] ?? null);
			$taxSumGroup->setSumGrossDefaultCurrency($dataset['t_sum_gross_default_currency'] ?? null);
			$taxSumGroup->setSumTaxDefaultCurrency($dataset['t_sum_tax_default_currency'] ?? null);
