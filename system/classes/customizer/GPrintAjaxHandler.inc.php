<?php
/* --------------------------------------------------------------
   GPrintAjaxHandler.inc.php 2023-07-10
   Gambio GmbH
   http://www.gambio.de
...
class GPrintAjaxHandler extends AjaxHandler
{
	...
	function proceed()
	{
        ...
        if (isset($this->v_data_array['POST']['modifiers']['property'])) {
            $this->v_data_array['POST']['properties_values_ids'] = $this->v_data_array['POST']['modifiers']['property'];
        }

        $idArray = [];
        
        if (isset($this->v_data_array['POST']['modifiers']['attribute'])) {
            foreach ($this->v_data_array['POST']['modifiers']['attribute'] as $optionId => $valueId) {
                $idArray[$optionId] = $valueId;
            }
            
            if(isset($this->v_data_array['POST']['id'])) {
                foreach ($this->v_data_array['POST']['id'] as $optionId => $valueId) {
                    $idArray[$optionId] = $valueId;
                }
            }

            $this->v_data_array['POST']['id'] = $idArray;
            
            $newProductId = (int)$this->v_data_array['POST']['products_id'];

            foreach ($idArray as $optionId => $valueId) {
                $newProductId .= '{' . $optionId . '}' . $valueId;
            }

            $this->v_data_array['POST']['products_id'] = $newProductId;
        }
		...
		elseif((!isset($_SESSION['coo_gprint']) || !is_object($_SESSION['coo_gprint'])) 
		...
		{
			case 'add_cart':
				if(!is_object($_SESSION['coo_gprint_cart'] ?? null))
				{
				...
				$_SESSION['coo_gprint_cart']->v_elements[$c_products_id] = $_SESSION['coo_gprint_cart']->v_elements[$c_products_id]
				                                                           ?? [];
			...
			case 'update_cart':
				...
				break;
			case 'add_wishlist':
				if(!is_object($_SESSION['coo_gprint_wishlist'] ?? null))
				{
				...
				$_SESSION['coo_gprint_wishlist']->v_elements[$c_products_id] = $_SESSION['coo_gprint_wishlist']->v_elements[$c_products_id]
				                                                               ?? [];
			...
			case 'create_surface':
				if($this->v_data_array['POST']['mode'] == 'backend')
				{
					...
					foreach($this->languageProvider->getIds()->getIntArray() as $languageId)
					{
						if(!array_key_exists((string)$languageId, $t_names))
						{
							$t_names[(string)$languageId] = $t_names[(string)$defaultLanguageId];
						}
					}
			...
			case 'create_element':
				if($this->v_data_array['POST']['mode'] == 'backend')
				{
					...
					foreach($this->languageProvider->getIds()->getIntArray() as $languageId)
					{
						...
						if(!array_key_exists((string)$languageId, $t_values))
						{
							$t_values[(string)$languageId] = $t_values[(string)$defaultLanguageId];
						}
					}
		...
		elseif(isset($this->v_data_array['GET']['action']) && $this->v_data_array['GET']['action'] == 'upload')
		{
			...
			if(!is_object($_SESSION['coo_gprint_cart'] ?? null) && $f_target == 'cart')
			{
				$_SESSION['coo_gprint_cart'] = new GMGPrintCartManager();
			}
			elseif(!is_object($_SESSION['coo_gprint_wishlist'] ?? null) && $f_target == 'wishlist')
			{
				$_SESSION['coo_gprint_wishlist'] = new GMGPrintWishlistManager();
			}
