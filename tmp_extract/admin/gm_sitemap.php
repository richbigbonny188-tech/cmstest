									...
									<div class="gx-container">
										<div class="ui-tabs">
											<div class="tab-headline-wrapper">
												<?php
												// Create Sitemap
												$class = (isset($_GET['action']) && $_GET['action'] == 'gm_sitemap') || !isset($_GET['action'])
													? 'active' : '';
												...
												// Configure Sitemap
												$class = (isset($_GET['action']) && $_GET['action'] === 'gm_sitemap_conf')
														? 'active' : '';
												...
												// Publish all in Sitemap
												$class = (isset($_GET['action']) && $_GET['action'] === 'gm_sitemap_publish_all')
														? 'active' : '';

												$href = xtc_href_link('gm_sitemap.php', 'action=gm_sitemap_publish_all');

												echo '<a class="' . $class .'" href="' . $href . '">'
												     . MENU_TITLE_GM_SITEMAP_PUBLISH_ALL .
												     '</a>';
													...
													<?php
														switch(($_GET['action'] ?? null)) {
															case 'gm_sitemap_conf':
																$gm_conf = gm_get_conf([
																	                       'GM_SITEMAP_GOOGLE_CHANGEFREQ',
																	                       'GM_SITEMAP_GOOGLE_PRIORITY'
																                       ]);
																include(DIR_FS_ADMIN . 'includes/gm/gm_sitemap/gm_sitemap_conf.php');
																break;

															case 'gm_sitemap_publish_all':
																include(DIR_FS_ADMIN . 'includes/gm/gm_sitemap/gm_sitemap_publish_all.php');
																break;
