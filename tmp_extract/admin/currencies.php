	...
	switch ($_GET['action']) {
		...
		case 'save':
			...
			if (($_POST['default'] ?? null) == 'on') {
				xtc_db_query("update `gx_configurations` set `value` = '" . xtc_db_input($code) . "' where `key` = 'configuration/DEFAULT_CURRENCY'");
			}
		...
		case 'update':
			...
			while ($currency = xtc_db_fetch_array($currency_query)) {
				...
				$rate = getCurrentRate($currency['code']);
									...
									<table border="0" width="100%" cellspacing="0" cellpadding="0">
										<tr>
											<td valign="top"><table class="gx-modules-table left-table" border="0" width="100%" cellspacing="0" cellpadding="2">
													...
													while ($currency = xtc_db_fetch_array($currency_query)) {
													  if ((empty($_GET['cID']) || $_GET['cID'] == $currency['currencies_id']) && empty($cInfo) && substr($_GET['action'] ?? '', 0, 3) !== 'new') {
												...
												<table class="gx-container paginator left-table table-paginator">
												    <tr>
														<td class="bottom-save-bar-content">
															<?php if (CURRENCY_SERVER_PRIMARY && (!isset($_GET['action']) || $_GET['action'] !== 'edit')) { echo '<a class="button" onClick="this.blur();" href="' . xtc_href_link(FILENAME_CURRENCIES, 'page=' . $_GET['page'] . '&cID=' . ($cInfo->currencies_id ?? null) . '&action=update') . '">' . BUTTON_UPDATE_EXCHANGE_RATES . '</a>'; } ?>
														</td>
				...
				<?php
					...
					switch ($_GET['action'] ?? null) {
						case 'new':
							...
							$formAction = xtc_href_link(FILENAME_CURRENCIES, 'page=' . $_GET['page'] . '&cID=' . ($cInfo->currencies_id ?? null) . '&action=insert');
							$formIsEditable = true;
