  ...
  if (!empty($_GET['action'])) {
    switch ($_GET['action']) {
      case 'update':
        ...
        $last_modified = xtc_db_prepare_input($_POST['last_modified'] ?? null);
...
<!-- body_text //-->
    <td class="boxCenter" width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="0" <?php if(isset($_GET['action']) && ($_GET['action'] === 'edit' || $_GET['action'] === 'preview')) { ?>class="dataTableRow breakpoint-small"<?php }?>>
      <tr>
		  <td>
...
<?php
  if (isset($_GET['action']) && $_GET['action'] === 'edit') {
    ...
    $products_name_query = xtc_db_query("select products_name from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . $reviews['products_id'] . "' and language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "'");
    $products_name = xtc_db_fetch_array($products_name_query);

    $rInfo_array = xtc_array_merge($reviews, $products, $products_name);
    $rInfo = new objectInfo($rInfo_array);
?>
      <tr><?php echo xtc_draw_form('review', FILENAME_REVIEWS, 'page=' . (int)($_GET['page'] ?? 0) . '&rID=' . (int)$_GET['rID'] . '&action=preview'); ?>
        ...
        <td class="main"><b><?php echo ENTRY_RATING; ?></b>&nbsp;<?php echo TEXT_BAD; ?>&nbsp;<?php for ($i=1; $i<=5; $i++) echo xtc_draw_radio_field('reviews_rating', $i, '', (int)$rInfo->reviews_rating) . '&nbsp;'; echo TEXT_GOOD; ?></td>
          ...
          <tr>
        <td align="left" class="main bottom-save-bar-content">
	        <?php
				  ...
				  <tr>
					  <td class="main"><b><?php echo ENTRY_RATING; ?></b>&nbsp;
						  <?php
						        ...
						        echo xtc_image(HTTP_SERVER . DIR_WS_CATALOG . StaticGXCoreLoader::getThemeControl()->getThemeImagePath()
						                 . $filename, sprintf(TEXT_OF_5_STARS, (int)$rInfo->reviews_rating));
						  ?>&nbsp;<small>[<?php echo sprintf(TEXT_OF_5_STARS, (int)$rInfo->reviews_rating); ?>]</small>
					  </td>
				  </tr>
			  </table>
		  </td>
...
<?php
    if ($_POST) {
      ...
      reset($_POST);
      foreach($_POST as $key => $value) echo '<input type="hidden" name="' . $key . '" value="' . htmlspecialchars_wrapper(stripslashes($value)) . '">';
?>
...
?>
      <tr>
        <td align="right" class="bottom-save-bar-content"><?php echo '<a class="btn" onClick="this.blur();" href="' . xtc_href_link($back_url, $back_url_params, 'NONSSL') . '">' . BUTTON_BACK . '</a>'; ?></td>
      </tr>
<?php
    }
  } else {
...
<?php
    ...
    while ($reviews = xtc_db_fetch_array($reviews_query)) {
      if ( key_exists('rID', $_GET) && (!$_GET['rID']) || (key_exists('rID', $_GET) && $_GET['rID'] == $reviews['reviews_id']) && (empty($rInfo)) ) {
...
?>
                <td class="dataTableContent"><?php echo htmlspecialchars(xtc_get_products_name($reviews['products_id'])); ?></td>
                <td class="dataTableContent" align="left">
	                <?php
		                $filename = 'stars_' . (int)$reviews['reviews_rating'] . '.gif';
		                
	                    echo xtc_image(HTTP_SERVER . DIR_WS_CATALOG . StaticGXCoreLoader::getThemeControl()->getThemeImagePath() . $filename);
	                ?>
                </td>
    ...
    <div class="hidden">
        ...
        switch ($_GET['action'] ?? '') {
            case 'delete':
                ...
                $buttons .= '<a class="btn" onClick="this.blur();" href="' . xtc_href_link(FILENAME_REVIEWS, 'page=' . (int)($_GET['page'] ?? 0) . '&rID=' . (int)$rInfo->reviews_id) . '">' . BUTTON_CANCEL . '</a>';
                break;

            default:
                if (isset($rInfo) && is_object($rInfo)) {
                    $editButton = '<a class="btn btn-primary btn-edit" href="' . xtc_href_link(FILENAME_REVIEWS, 'page=' . (int)($_GET['page'] ?? 0) . '&rID=' . (int)$rInfo->reviews_id . '&action=edit') . '">' . BUTTON_EDIT . '</a>';
                    $deleteButton = '<a class="btn btn-delete" href="' . xtc_href_link(FILENAME_REVIEWS, 'page=' . (int)($_GET['page'] ?? 0) . '&rID=' . (int)$rInfo->reviews_id . '&action=delete') . '">' . BUTTON_DELETE . '</a>';
                    $previewButton = '<a class="btn" href="' . xtc_href_link(FILENAME_REVIEWS, 'page=' . (int)($_GET['page'] ?? 0) . '&rID=' . (int)$rInfo->reviews_id . '&action=preview') . '">' . BUTTON_PREVIEW . '</a>';
                    ...
                    if (property_exists($rInfo, 'last_modified') && xtc_not_null($rInfo->last_modified)) $contents[] = array('text' => '<span class="options-title">' . TEXT_INFO_LAST_MODIFIED . '</span>' . xtc_date_short($rInfo->last_modified));
	                ...
	                $contents[] = array('text' => '<span class="options-title">' . TEXT_INFO_REVIEW_RATING . '</span>' . xtc_image(HTTP_SERVER . DIR_WS_CATALOG . StaticGXCoreLoader::getThemeControl()->getThemeImagePath() . $filename));
                    $contents[] = array('text' => '<span class="options-title">' . TEXT_INFO_REVIEW_READ . '</span>' . (property_exists($rInfo, 'reviews_read') ? $rInfo->reviews_read : ''));
                    $contents[] = array('text' => '<span class="options-title">' . TEXT_INFO_REVIEW_SIZE . '</span>' . (property_exists($rInfo, 'reviews_text_size') ? $rInfo->reviews_text_size . ' bytes' : ''));
                    $contents[] = array('text' => '<span class="options-title">' . TEXT_INFO_PRODUCTS_AVERAGE_RATING . '</span>' . (property_exists($rInfo, 'average_rating') ? number_format((double)$rInfo->average_rating, 2) . '%' : ''));
