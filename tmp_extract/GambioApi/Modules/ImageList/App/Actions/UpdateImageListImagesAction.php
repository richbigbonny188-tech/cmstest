<?php
/*--------------------------------------------------------------
   UpdateImageListImagesAction.php 2021-09-17
   Gambio GmbH
   http://www.gambio.de
...
class UpdateImageListImagesAction
{
    ...
    {
        $errors = $this->validator->validateUpdateImageRequest($parsedBody = $request->getParsedBody());
        ...
        try {
            ...
            foreach ($parsedBody as ['relativePath' => $relativePath, 'sortOrder' => $sortOrder, 'titles' => $titles, 'altTitles' => $altTitles]) {
                ...
                // will throw a ImageDoesNotExistException if image does not belong to this list
                $imageList->images()->getByLocalPath($localPath);
