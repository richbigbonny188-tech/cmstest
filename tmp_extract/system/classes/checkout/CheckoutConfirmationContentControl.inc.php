<?php
/* --------------------------------------------------------------
  CheckoutConfirmationContentControl.inc.php 2023-06-27
  Gambio GmbH
  http://www.gambio.de
...
class CheckoutConfirmationContentControl extends CheckoutControl
{
	public function proceed()
	{
		...
		if(gm_get_conf('GM_CHECK_WITHDRAWAL') == 1 && !($this->v_data_array['POST']['withdrawal'] ?? null))
		{
        ...
        if ((!isset($_SESSION['conditions']) && gm_get_conf('GM_CHECK_CONDITIONS') == 1)
			...
			if((!($this->v_data_array['POST']['conditions'] ?? null) && gm_get_conf('GM_CHECK_CONDITIONS') == 1)
				|| (!($this->v_data_array['POST']['withdrawal'] ?? null) && gm_get_conf('GM_CHECK_WITHDRAWAL') == 1)
				|| !$transportConditionsSuccess
			)
			{
			...
			{
				if($this->v_data_array['POST']['conditions'] ?? false)
				{
					$_SESSION['conditions'] = 'true';
				}

				if($this->v_data_array['POST']['withdrawal'] ?? false)
				{
