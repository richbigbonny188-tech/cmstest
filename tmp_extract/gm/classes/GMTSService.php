...
class GMTSService_ORIGIN {
	...
	protected $_languages = array('de', 'en', 'es', 'fr', 'pl', 'it', 'nl');
	...
	protected function _log($text) {
	    ...
	    if ($logger === null) {
            ...
            $logger = $loggerBuilder->omitRequestData()->changeNamespace('trusted_shops')->build();
        }
	    $logger->notice($text);
	}
		...
		if(is_object($snippet_data))
		{
			...
			if($reviewsFound)
			{
				...
				if($this->_sandboxmode == true)
				{
					if($cache_used)
					{
						$snippet .= '<!-- cached data, retrieved '.$cachefile_mtime.' -->';
					}
					else
					{
						$snippet .= '<!-- freshly retrieved data -->';
					}
				}
			}
			else
