...
{
	public $v_connect_id = null;
	public $v_error = '';
	public $v_data_array = array();
	public $v_wrong_chmod_array = array();
	protected $isDirCache = array();
	protected $additionalChmodAll = array(
		'/admin/includes/magnalister',
	);
	
	public function __construct($p_connect = true, $p_host, $p_user, $p_password, $p_pasv = true)
	{
		if($p_connect == true)
		{
	...
	public function connect($p_host, $p_user, $p_password, $p_pasv)
	{
	...
	public function get_dir_content($p_dir)
	{
		$t_dir_content = ftp_nlist($this->v_connect_id, $p_dir);
	...
	public function chmod_777($p_dir)
	{
		...
		while($t_line = fgets($fp, 1024)) {
			...
			if(strlen($t_line) > 0)
			{
				...
				$this->recursive_ftpn_list($t_line, true, false);
	...
	public function recursive_ftpn_list($p_dir, $p_directories = true, $p_files = true, $p_exclude = array('.', '..'))
	{
		$t_list_array = ftp_nlist($this->v_connect_id, $p_dir);
		for($i = 0; $i < count($t_list_array); $i++)
		{
	...
	{
		$this->v_wrong_chmod_array = SecurityCheck::getWrongPermittedInstallerFiles();

		return $this->v_wrong_chmod_array;
	}
