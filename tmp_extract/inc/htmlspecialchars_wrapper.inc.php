<?php
/* --------------------------------------------------------------
   htmlspecialchars_wrapper.inc.php 2022-08-10 gm
   Gambio GmbH
   http://www.gambio.de
...
function htmlspecialchars_wrapper($p_string, $p_flags = false, $p_encoding = '', $p_double_encode = true)
{
	...
	if($p_encoding === '')
	{
        ...
        if(preg_match('//u', (string)$p_string))
		{
			$t_encoding = 'UTF-8';
		}
	...
	{
		return htmlspecialchars((string)$p_string, $t_flags, $t_encoding);
	}
	else
	{
		return htmlspecialchars((string)$p_string, $t_flags, $t_encoding, $p_double_encode);
	}
}
