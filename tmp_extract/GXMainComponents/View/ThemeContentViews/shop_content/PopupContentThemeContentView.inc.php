<?php
/* --------------------------------------------------------------
  PopupContentThemeContentView.inc.php 2021-05-04
  Gambio GmbH
  http://www.gambio.de
...
class PopupContentThemeContentView extends ThemeContentView
{
    ...
    {
        parent::__construct();
        $this->set_content_template('popup_content.html');
    ...
    protected function set_validation_rules()
    {
        ...
        $this->validation_rules_array['lightbox_mode']      = ['type' => 'int'];
    ...
    {
        $t_uninitialized_array = $this->get_uninitialized_variables(['content_group_id']);
    ...
    protected function get_data()
    {
        ...
        $this->content_data_array['withdrawalContents'] = [];
        
        // CHECK IF CONTENT FILE IS SET
        if ($this->content_data_array['content_file'] != ''
            && file_exists(DIR_FS_CATALOG . 'media/content/' . basename($this->content_data_array['content_file']))) {
            ...
            $this->content_data_array['content_text'] = str_replace('{$shipping_and_payment_matrix}',
                                                                    $t_matrix_content,
                                                                    $this->content_data_array['content_text']);
            ...
            while ($t_row = xtc_db_fetch_array($t_shop_content_query)) {
                if ($t_row['content_file'] != ''
                    && file_exists(DIR_FS_CATALOG . 'media/content/' . basename($t_row['content_file']))) {
                    $withdrawalContents[] = [
                        'heading' => null,
                        'html'    => $this->load_content_file($t_row['content_file']),
                    ];
                } else {
                    $withdrawalContents[] = [
                        'heading' => $t_row['content_heading'],
                        'html'    => $t_row['content_text'],
                    ];
                }
            }
            
            $this->content_data_array['withdrawalContents'] = $withdrawalContents;
        }
    }
    ...
    protected function set_data()
    {
        ...
        $this->content_array['BASE_HREF']           = GM_HTTP_SERVER . DIR_WS_CATALOG;
        $this->content_array['LIGHTBOX_MODE']       = $this->lightbox_mode;
        $this->content_array['HTML']                = $this->content_data_array['content_text'];
        $this->content_array['WITHDRAWAL_CONTENTS'] = $this->content_data_array['withdrawalContents'];
        
        $headerContentView = MainFactory::create(HeaderThemeContentView::class);
        
        if (file_exists(DIR_FS_CATALOG . 'cache/__dynamics.css')) {
            $this->content_array['MAIN_CSS'] = $headerContentView->getMainCss();
        } else {
            $this->content_array['CSS_FILE']   = $headerContentView->getCssFile();
            $this->content_array['CSS_PARAMS'] = implode('&amp;', $headerContentView->getCssParams());
        }
    }
