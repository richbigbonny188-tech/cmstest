<?php
/* --------------------------------------------------------------
   OrdersTotalsApiV2Controller.inc.php 2021-07-26
   Gambio GmbH
   http://www.gambio.de
...
class OrdersTotalsApiV2Controller extends HttpApiV2Controller
{
    ...
    {
        $orderTotalJsonString = json_encode($this->request->getParsedBody());
        
        if (empty($orderTotalJsonString) || $this->request->getParsedBody() === null) {
     ...
     *             curl -i --user admin@example.org:12345 https://example.org/api.php/v2/orders/400785/totals
     *
     * @apiExample {curl} Get Entry With ID = 4
     *             curl -i --user admin@example.org:12345 https://example.org/api.php/v2/orders/400242/totals/4
     */
    public function get()
    {
        $orderId = new IdType($this->uri[1]);
