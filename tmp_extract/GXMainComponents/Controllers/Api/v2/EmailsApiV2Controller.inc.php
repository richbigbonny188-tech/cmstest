<?php
/* --------------------------------------------------------------
   EmailsApiV2Controller.inc.php 2021-07-26
   Gambio GmbH
   http://www.gambio.de
...
class EmailsApiV2Controller extends HttpApiV2Controller
{
    ...
    {
        $emailJsonString = json_encode($this->request->getParsedBody());
        
        // Resend existing email without any changes.
        if (isset($this->uri[1]) && is_numeric($this->uri[1])
            && (empty($emailJsonString)
                || $this->request->getParsedBody() === null)) {
            ...
            if (empty($emailJsonString) || $this->request->getParsedBody() === null) {
     ...
     * curl -X DELETE --user admin@example.org:12345 https://example.org/api.php/v2/emails/572
     *
     ...
     *             curl -i --user admin@example.org:12345 https://example.org/api.php/v2/emails
     *
     * @apiExample {curl} Get Email With ID = 527
     *             curl -i --user admin@example.org:12345 https://example.org/api.php/v2/emails/527
     *
     * @apiExample {curl} Get Pending Emails
     *             curl -i --user admin@example.org:12345 https://example.org/api.php/v2/emails?state=pending
     *
     * @apiExample {curl} Search Emails
     *             curl -i --user admin@example.org:12345 https://example.org/api.php/v2/emails?q=admin@example.org
     *
    ...
    public function get()
    {
        ...
        if (isset($this->uri[1]) && is_numeric($this->uri[1])) {
            ...
            if ($searchTerm !== null) {
                $emails = $this->emailService->filter($searchTerm);
            } // Filter results by state ("pending" or "sent").
            else {
                if ($this->request->getQueryParam('state') !== null) {
                    $emails = ($this->request->getQueryParam('state')
