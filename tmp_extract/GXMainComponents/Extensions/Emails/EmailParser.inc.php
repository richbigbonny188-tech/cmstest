<?php
/* --------------------------------------------------------------
   EmailParser.inc.php 2021-04-09 gm
   Gambio GmbH
   http://www.gambio.de
...
class EmailParser
{
    ...
    public function parseEmail(array $encodedEmail)
    {
        ...
        if (!empty($encodedEmail['creation_date'])) {
            $email->setCreationDate(new DateTime($encodedEmail['creation_date']));
        }
    ...
    {
        $encodedCollection = []; // Contains final result.
    ...
    public function encodeAttachment(EmailAttachmentInterface $attachment)
    {
        return [
            ...
            'file_exists' => file_exists((string)$attachment->getPath())
            // determines whether the file exists in the server
        ];
    }
}