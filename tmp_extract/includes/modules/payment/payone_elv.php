...
require_once __DIR__ . '/payone/payone_master.php';

class payone_elv_ORIGIN extends payone_master
{
    public $payone_genre = 'accountbased';
    ...
    {
        $standard_parameters                 = $this->payone->getStandardParameters('bankaccountcheck',
                                                                                    $this->global_config);
        ...
        $standard_parameters['hash'] = $this->payone->computeHash($standard_parameters,
                                                                  $this->global_config['key'],
                                                                  $this->global_config['hash_algo']);
        ...
        include DIR_FS_CATALOG . 'themes/Honeygrid/html/system/checkout_payment_payone_elv_form.html';
    ...
    function _getAddressBookIso2($ab_id)
    {
        ...
        $t_query  = strtr($t_query, [':ab_id' => (int)$ab_id]);
    ...
    function pre_confirmation_check()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_POST['p1_elv_accountnumber']) && empty($_POST['p1_elv_accountnumber']) === false
                && isset($_POST['p1_elv_bankcode'])
                && empty($_POST['p1_elv_bankcode']) === false) {
            ...
            if (isset($_POST['p1_elv_iban']) && empty($_POST['p1_elv_iban']) === false && isset($_POST['p1_elv_bic'])
                && empty($_POST['p1_elv_bic']) === false) {
                $accountnumber = '';
                $bankcode      = '';
        ...
        if (empty($_SESSION['payone_elv_data']['accountnumber']) !== true
            && empty($_SESSION['payone_elv_data']['bankcode']) !== true) {
            $t_has_accbank = true;
        } else {
            $t_has_accbank = false;
        }
        
        if (empty($_SESSION['payone_elv_data']['iban']) !== true
            && empty($_SESSION['payone_elv_data']['bic']) !== true) {
            $t_has_sepa = true;
        } else {
            $t_has_sepa = false;
        }
        
        if (($t_has_accbank === true || $t_has_sepa === true) !== true
            || empty($_SESSION['payone_elv_data']['country']) === true) {
    ...
    function process_button()
    {
        ...
        if ($this->pg_config['genre_specific']['sepa_use_managemandate'] == 'true') {
            ...
            $mmr_personal_data->setStreet($order->billing['street_address']
                                          . (empty($order->billing['house_number']) ? '' : ' '
            ...
            if ($manage_mandate_result instanceof Payone_Api_Response_Error) {
                $error_msg = "ERROR retrieving SEPA mandate: " . $manage_mandate_result->getErrorcode() . ' - '
                             . $manage_mandate_result->getErrormessage();
        ...
        if (isset($_SESSION['tmp_oID']) === false) {
            ...
            if ($this->pg_config['genre_specific']['sepa_use_managemandate'] == 'true') {
                if (isset($_POST['mandate_confirm']) !== true
                    ...
                    xtc_redirect(GM_HTTP_SERVER . DIR_WS_CATALOG
                                 . 'checkout_confirmation.php?payment_error=must_confirm_sepa_mandate&ret_errormsg='
                                 . $error_msg);
                }
            }
        }
    ...
    function payment_action()
    {
        ...
        $request             = $this->global_config['authorization_method']
                               == 'auth' ? 'authorization' : 'preauthorization';
        $standard_parameters = $this->payone->getStandardParameters($request, $this->global_config);
        unset($standard_parameters['responsetype']);
        unset($standard_parameters['successurl']);
        unset($standard_parameters['errorurl']);
        ...
        if ($response instanceof Payone_Api_Response_Preauthorization_Approved) {
            ...
            $this->payone->saveTransaction($GLOBALS['insert_id'],
                                           $response->getStatus(),
                                           $response->getTxid(),
                                           $response->getUserid());
            $this->_updateOrdersStatus($orders_id,
                                       $response->getTxid(),
                                       strtolower((string)$response->getStatus()),
                                       $this->payone->get_text('comment_preauth_approved'));
            ...
            $this->_updateOrdersStatus($orders_id,
                                       $response->getTxid(),
                                       strtolower((string)$response->getStatus()),
                                       $this->payone->get_text('comment_auth_approved'));
        } elseif ($response instanceof Payone_Api_Response_Authorization_Redirect) {
            $this->payone->log("authorization for order " . $GLOBALS['insert_id'] . " initiated, txid = "
                               . $response->getTxid());
            if ($response->getStatus() == 'REDIRECT') {
                ...
                $this->_updateOrdersStatus($orders_id,
                                           $response->getTxid(),
                                           strtolower((string)$response->getStatus()),
                                           $this->payone->get_text('comment_redirection_initiated'));
                $redirect_url = $response->getRedirecturl();
                xtc_redirect($redirect_url);
            }
        } elseif ($response instanceof Payone_Api_Response_Error) {
            $this->payone->log("authorization for order " . $GLOBALS['insert_id'] . " failed, status "
                               . $response->getStatus() . ", code " . $response->getErrorcode() . ", message "
                               . $response->getErrormessage());
            $this->_updateOrdersStatus($orders_id,
                                       0,
                                       strtolower((string)$response->getStatus()),
                                       $this->payone->get_text('comment_error'));
