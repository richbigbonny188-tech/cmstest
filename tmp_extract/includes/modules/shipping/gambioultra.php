...
class gambioultra_ORIGIN
{
    ...
    public function __construct()
    {
        ...
        $this->title       = defined('MODULE_SHIPPING_GAMBIOULTRA_TEXT_TITLE') ? MODULE_SHIPPING_GAMBIOULTRA_TEXT_TITLE : '';
        $this->description = defined('MODULE_SHIPPING_GAMBIOULTRA_TEXT_DESCRIPTION') ? MODULE_SHIPPING_GAMBIOULTRA_TEXT_DESCRIPTION : '';
        $this->sort_order  = defined('MODULE_SHIPPING_GAMBIOULTRA_SORT_ORDER') ? MODULE_SHIPPING_GAMBIOULTRA_SORT_ORDER : '0';
        $this->icon        = '';
        $this->tax_class   = defined('MODULE_SHIPPING_GAMBIOULTRA_TAX_CLASS') ? MODULE_SHIPPING_GAMBIOULTRA_TAX_CLASS : '0';
    ...
    {
        global $order, $shipping_weight, $xtPrice, $shipping_num_boxes;
        ...
        if ($dest_zone == 0) {
            ...
            for ($i = 0; $i < $size; $i += 2) {
                if ($shipping_weight <= (float)$zones_table[$i]) {
                    $shipping        = $zones_table[$i + 1] ?? null;
            ...
            if ($shipping == -1) {
                ...
                $zones_cost_full = ($shipping * $shipping_num_boxes) + (double)constant('MODULE_SHIPPING_GAMBIOULTRA_HANDLING_' . $dest_zone);
                $shipping_cost   = $zones_cost_full + $nc_ultra_data['costs'];
                if ($zones_cost_full > 0) {
                    $shipping_method .= $xtPrice->xtcFormat($zones_cost_full, true, $this->tax_class, true)
                                        . $nc_ultra_details;
                } else {
        ...
        if ($error == true) {
            $this->quotes['error'] = MODULE_SHIPPING_GAMBIOULTRA_UNDEFINED_RATE;
        }
        ...
        for ($i = 0; $i < sizeof($products); $i++) {
            ...
            if (((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_errno($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_errno()) ? $___mysqli_res : false))
                ...
                while (($row = mysqli_fetch_array($result))) {
                    ...
                    $infos[] = [
                        'title' => gm_prepare_number($products[$i]['quantity'],
                                                     $xtPrice->currencies[$xtPrice->actualCurr]['decimal_point']) . 'x '
                                   . $row['products_name'],
                        'price' => $xtPrice->xtcFormat($row['costs'] * $products[$i]['quantity'],
                                                       true,
                                                       $this->tax_class,
                                                       true)
                    ];
    ...
    function check()
    {
        if (!isset($this->_check)) {
            $check_query  = xtc_db_query("SELECT `value` from `gx_configurations` where `key` = 'configuration/MODULE_SHIPPING_GAMBIOULTRA_STATUS'");
    ...
    function install()
    {
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_SHIPPING_GAMBIOULTRA_STATUS', 'True', '0', 'switcher', now())");
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_GAMBIOULTRA_ALLOWED', '', '0', now())");
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_SHIPPING_GAMBIOULTRA_TAX_CLASS', '0', '0', 'tax-class', now())");
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_GAMBIOULTRA_SORT_ORDER', '0', '0', now())");
        for ($i = 1; $i <= $this->num_zones; $i++) {
            $default_countries = '';
            if ($i == 1) {
                $default_countries = 'DE,CH,AT';
            }
            xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_GAMBIOULTRA_COUNTRIES_"
                         . $i . "', '" . $default_countries . "', '0', now())");
            xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_GAMBIOULTRA_COST_"
                         . $i . "', '', '0', now())");
            xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_GAMBIOULTRA_HANDLING_"
                         . $i . "', '0', '0', now())");
        }
    }
    ...
    {
        $keys = [
            'configuration/MODULE_SHIPPING_GAMBIOULTRA_STATUS',
            'configuration/MODULE_SHIPPING_GAMBIOULTRA_ALLOWED',
            'configuration/MODULE_SHIPPING_GAMBIOULTRA_TAX_CLASS',
            'configuration/MODULE_SHIPPING_GAMBIOULTRA_SORT_ORDER'
        ];
        
        for ($i = 1; $i <= $this->num_zones; $i++) {
            $keys[] = 'configuration/MODULE_SHIPPING_GAMBIOULTRA_COUNTRIES_' . $i;
            $keys[] = 'configuration/MODULE_SHIPPING_GAMBIOULTRA_COST_' . $i;
            $keys[] = 'configuration/MODULE_SHIPPING_GAMBIOULTRA_HANDLING_' . $i;
        }
