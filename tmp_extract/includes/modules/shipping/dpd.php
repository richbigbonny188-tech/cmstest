...
class dpd_ORIGIN
{
    ...
    {
        $this->code        = 'dpd';
        $this->title       = defined('MODULE_SHIPPING_DPD_TEXT_TITLE') ? MODULE_SHIPPING_DPD_TEXT_TITLE : '';
        $this->description = defined('MODULE_SHIPPING_DPD_TEXT_DESCRIPTION') ? MODULE_SHIPPING_DPD_TEXT_DESCRIPTION : '';
        $this->sort_order  = defined('MODULE_SHIPPING_DPD_SORT_ORDER') ? MODULE_SHIPPING_DPD_SORT_ORDER : '0';
        $this->icon        = '';
        $this->tax_class   = defined('MODULE_SHIPPING_DPD_TAX_CLASS') ? MODULE_SHIPPING_DPD_TAX_CLASS : '0';
    ...
    function quote($method = '')
    {
        ...
        if ($dest_zone == 0) {
            ...
            for ($i = 0; $i < $size; $i += 2) {
                if ($shipping_weight <= $zones_table[$i]) {
                    ...
                    $shipping_method = MODULE_SHIPPING_DPD_TEXT_WAY . ' ' . $dest_country . ': (' . $shipping_num_boxes
                                       . ' x ' . $shipping_weight . ' ' . MODULE_SHIPPING_DPD_TEXT_UNITS . ')';
                    break;
                }
            }
            ...
            if ($shipping == -1) {
                ...
                $shipping_cost = ($shipping + (double)constant('MODULE_SHIPPING_DPD_HANDLING_' . $dest_zone));
            }
        }
        ...
        $this->quotes = [
            ...
            'methods' => [
                [
                    'id'    => $this->code,
                    'title' => $shipping_method ?? null,
                    'cost'  => ($shipping_cost ?? null) * $shipping_num_boxes
                ]
            ]
        ];
        ...
        if ($error == true) {
            $this->quotes['error'] = MODULE_SHIPPING_DPD_UNDEFINED_RATE;
        }
    ...
    function check()
    {
        if (!isset($this->_check)) {
            $check_query  = xtc_db_query("SELECT `value` from `gx_configurations` where `key` = 'configuration/MODULE_SHIPPING_DPD_STATUS'");
    ...
    function install()
    {
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_SHIPPING_DPD_STATUS', 'True', '0', 'switcher', now())");
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_DPD_ALLOWED', '', '0', now())");
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `type`, `last_modified`) values ('configuration/MODULE_SHIPPING_DPD_TAX_CLASS', '0', '0', 'tax-class', now())");
        xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_DPD_SORT_ORDER', '0', '0', now())");
        for ($i = 1; $i <= $this->num_zones; $i++) {
            $default_countries = '';
            if ($i == 1) {
                $default_countries = 'US,CA';
            }
            xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_DPD_COUNTRIES_"
                         . $i . "', '" . $default_countries . "', '0', now())");
            xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_DPD_COST_"
                         . $i . "', '3:8.50,7:10.50,99:20.00', '0', now())");
            xtc_db_query("insert into `gx_configurations` (`key`, `value`, `sort_order`, `last_modified`) values ('configuration/MODULE_SHIPPING_DPD_HANDLING_"
                         . $i . "', '0', '0', now())");
        }
    }
    ...
    {
        $keys = [
            'configuration/MODULE_SHIPPING_DPD_STATUS',
            'configuration/MODULE_SHIPPING_DPD_ALLOWED',
            'configuration/MODULE_SHIPPING_DPD_TAX_CLASS',
            'configuration/MODULE_SHIPPING_DPD_SORT_ORDER'
        ];
        
        for ($i = 1; $i <= $this->num_zones; $i++) {
            $keys[] = 'configuration/MODULE_SHIPPING_DPD_COUNTRIES_' . $i;
            $keys[] = 'configuration/MODULE_SHIPPING_DPD_COST_' . $i;
            $keys[] = 'configuration/MODULE_SHIPPING_DPD_HANDLING_' . $i;
        }
