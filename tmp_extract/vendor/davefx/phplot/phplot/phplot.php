...
/////////////////////////////////////////////
    ...
    protected function GetColorIndex(&$color, $default_color_index = 0)
    {
        ...
        [$r, $g, $b, $a] = $color;
        return imagecolorresolvealpha($this->img, (int)$r, (int)$g, (int)$b, (int)$a);
    }
...
/////////////////////////////////////////////
    ...
    function DrawMessage($text, $options = NULL)
    {
        ...
        if ($draw_background) {  // User-specified background
            ...
            ImageFilledRectangle($this->img, 0, 0, (int)$this->image_width, (int)$this->image_height, (int)$bgcolor);
        }
        if ($draw_border) $this->DrawImageBorder(TRUE);
        if (empty($text_color)) $rgb = array(0, 0, 0);
        else $rgb = $this->SetRGBColor($text_color);
        $ndx_text_color = imagecolorresolve($this->img, (int)$rgb[0], (int)$rgb[1], (int)$rgb[2]);
        ...
        [$text_width, $text_height] = $this->SizeText('generic', 0, $text);
...
//////////////////////////////////////////////////////////
    ...
    protected function CalcMargins($maximize)
    {
        ...
        [$unused, $title_height] = $this->SizeText('title', 0, $this->title_txt);
        ...
        [$unused, $x_title_height] = $this->SizeText('x_title', 0, $this->x_title_txt);
        [$y_title_width, $unused] = $this->SizeText('y_title', 90, $this->y_title_txt);
    ...
    {
        [$this->x_tick_inc, $this->plot_min_x, $this->plot_max_x] = $this->CalcPlotRange('x');
        [$this->y_tick_inc, $this->plot_min_y, $this->plot_max_y] = $this->CalcPlotRange('y');
        if ($this->GetCallback('debug_scale')) {
            $this->DoCallback('debug_scale', __FUNCTION__, array(
    ...
    {
        [$tick_start, $tick_end, $tick_step] = $this->CalcTicks($which);
        ...
        while ($tick_val <= $tick_end) {
            ...
            [$width, $height] = $this->SizeText($font_id, $angle, $tick_label);
    ...
    protected function CalcMaxDataLabelSize($which = 'x')
    {
        ...
        for ($i = 0; $i < $this->num_data_rows; $i++) {
            ...
            [$width, $height] = $this->SizeText($font_id, $angle, $label);
            if ($width > $max_width) $max_width = $width;
            if ($height > $max_height) $max_height = $height;
        }
        if ($this->GetCallback('debug_scale')) {
...
/////////////////////////////////////////////
    ...
    protected function tile_img($file, $xorig, $yorig, $width, $height, $mode)
    {
        ...
        if ($mode == 'scale') {
            imagecopyresampled($this->img, $im, (int)$xorig, (int)$yorig, 0, 0, (int)$width, (int)$height,
        ...
        $tmp = imagecreate((int)$width, (int)$height);
        if (! $tmp)
        ...
        for ($x = $x0; $x < $width; $x += $tile_width)
            for ($y = $y0; $y < $height; $y += $tile_height)
                imagecopy($tmp, $im, (int)$x, (int)$y, 0, 0, (int)$tile_width, (int)$tile_height);

        // Copy the temporary image onto the final one.
        imagecopy($this->img, $tmp, (int)$xorig, (int)$yorig, 0,0, (int)$width, (int)$height);
    ...
    protected function DrawImageBorder($overwrite=FALSE)
    {
        ...
        case 'raised':
            ...
            for ($i = 0; $i < $width; $i++, $ex--, $ey--) {
                imageline($this->img, $i, $i, (int)$ex, $i, (int)$color1); // Top
                imageline($this->img, (int)$ex, $i, (int)$ex, (int)$ey, (int)$color2); // Right
                imageline($this->img, $i, $i, $i, (int)$ey, (int)$color1); // Left
                imageline($this->img, $i, (int)$ey, (int)$ex, (int)$ey, (int)$color2); // Bottom
            }
            break;
        ...
        case 'solid':
            for ($i = 0; $i < $width; $i++, $ex--, $ey--) {
                imagerectangle($this->img, $i, $i, (int)$ex, (int)$ey, (int)$color1);
            }
            break;
        default:
    ...
    protected function DrawXTicks()
    {
        ...
        [$x_start, $x_end, $delta_x] = $this->CalcTicks('x');
        ...
        for ($n = 0; ($x = $x_start + $n * $delta_x) <= $x_end; $n++) {
            ...
            if ($draw_grid) {
                ImageLine($this->img, (int)$x_pixels, (int)$this->plot_area[1], (int)$x_pixels, (int)$this->plot_area[3], $style);
            }
    ...
    protected function DrawYTicks()
    {
        ...
        // Calculate the tick start, end, and step:
        [$y_start, $y_end, $delta_y] = $this->CalcTicks('y');
    ...
    protected function DrawPlotBorder($draw_axes = TRUE)
    {
        ...
        if ($sides == 15) { // Border on all 4 sides
            imagerectangle($this->img, (int)$this->plot_area[0], (int)$this->plot_area[1],
                           (int)$this->plot_area[2], (int)$this->plot_area[3], (int)$this->ndx_grid_color);
        } else {
            if ($sides & 1) // Left
                imageline($this->img, (int)$this->plot_area[0], (int)$this->plot_area[1],
                                      (int)$this->plot_area[0], (int)$this->plot_area[3], (int)$this->ndx_grid_color);
            if ($sides & 2) // Top
                imageline($this->img, (int)$this->plot_area[0], (int)$this->plot_area[1],
                                      (int)$this->plot_area[2], (int)$this->plot_area[1], (int)$this->ndx_grid_color);
            if ($sides & 4) // Right
                imageline($this->img, (int)$this->plot_area[2], (int)$this->plot_area[1],
                                      (int)$this->plot_area[2], (int)$this->plot_area[3], (int)$this->ndx_grid_color);
            if ($sides & 8) // Bottom
                imageline($this->img, (int)$this->plot_area[0], (int)$this->plot_area[3],
                                      (int)$this->plot_area[2], (int)$this->plot_area[3], (int)$this->ndx_grid_color);
        }
        return TRUE;
    }
    ...
    protected function DrawDataValueLabel($x_or_y, $row, $column, $x_world, $y_world, $text, $dvl)
    {
        ...
        if (isset($min_width) || isset($min_height)) {
            [$width, $height] = $this->SizeText($font_id, $angle, $formatted_text);
    ...
    protected function DrawXDataLine($xpos, $row)
    {
        ...
        if ($this->x_data_label_pos == 'both') {
            ...
            ImageLine($this->img, (int)$xpos, (int)$this->plot_area[3], (int)$xpos, (int)$this->plot_area[1], (int)$style);
            ...
            ImageLine($this->img, (int)$xpos, (int)$ypos, (int)$xpos, (int)$this->plot_area[3], (int)$style);
            ...
            ImageLine($this->img, (int)$xpos, (int)$this->plot_area[1], (int)$xpos, (int)$ypos, (int)$style);
        }
        return TRUE;
    }
    ...
    protected function DrawYDataLine($ypos, $row)
    {
        ...
        if ($this->y_data_label_pos == 'both') {
            ...
            ImageLine($this->img, (int)$this->plot_area[0], (int)$ypos, (int)$this->plot_area[2], (int)$ypos, (int)$style);
            ...
            ImageLine($this->img, (int)$xpos, (int)$ypos, (int)$this->plot_area[0], (int)$ypos, (int)$style);
            ...
            ImageLine($this->img, (int)$this->plot_area[2], (int)$ypos, (int)$xpos, (int)$ypos, (int)$style);
        }
        return TRUE;
    }
...
/////////////////////////////////////////////
    ...
    protected function GetLegendSizeParams()
    {
        ...
        foreach ($this->legend as $line) {
            [$width, $unused] = $this->SizeText('legend', 0, $line);
            if ($width > $max_width) $max_width = $width;
        }
    ...
    protected function GetLegendPosition($width, $height)
    {
        ...
        case 'title': // SetLegendPosition with mode='title', relative to main title.
            ...
            [$title_width, $title_height] = $this->SizeText('title', 0, $this->title_txt);
    ...
    protected function DrawLegend()
    {
        ...
        [$box_start_x, $box_start_y] = $this->GetLegendPosition($width, $height);
        ...
        foreach ($this->legend as $leg) {
            ...
            if ($colorbox_mode) {
                ...
                case 'line':
                    ...
                    imageline($this->img, $dot_left_x, (int)$yc, $dot_right_x, (int)$yc, $style);
                ...
                default:
                    // Draw color boxes:
                    ImageFilledRectangle($this->img, $dot_left_x, (int)$y1, $dot_right_x, (int)$y2,
                   ...
                   if ($this->legend_colorbox_borders != 'none') {
                       ...
                       ImageRectangle($this->img, $dot_left_x, (int)$y1, $dot_right_x, (int)$y2, (int)$color);
                   }
                }
...
/////////////////////////////////////////////
    ...
    protected function DrawShape($x, $y, $record, $color, $allow_none = TRUE)
    {
        ...
        case 'halfline':
            ImageLine($this->img, $x1, (int)$y, (int)$x, (int)$y, (int)$color);
            break;
        case 'none': /* Special case, no point shape here */
            if ($allow_none) break; // But fall throught to line if a shape is required
        case 'line':
            ImageLine($this->img, $x1, (int)$y, $x2, (int)$y, (int)$color);
            break;
        case 'plus':
            ImageLine($this->img, (int)$x1, (int)$y, (int)$x2, (int)$y, (int)$color);
            ImageLine($this->img, (int)$x, (int)$y1, (int)$x, (int)$y2, (int)$color);
            break;
        case 'cross':
            ImageLine($this->img, (int)$x1, (int)$y1, (int)$x2, (int)$y2, (int)$color);
            ImageLine($this->img, (int)$x1, (int)$y2, (int)$x2, (int)$y1, (int)$color);
            break;
        case 'circle':
            ImageArc($this->img, (int)$x, (int)$y, (int)$point_size, (int)$point_size, 0, 360, (int)$color);
            break;
        case 'dot':
            ImageFilledEllipse($this->img, (int)$x, (int)$y, (int)$point_size, (int)$point_size, (int)$color);
            break;
        case 'diamond':
            $arrpoints = array($x1, $y, $x, $y1, $x2, $y, $x, $y2);
            $this->imageFilledPolygon($this->img, $arrpoints, 4, (int)$color);
            break;
        case 'triangle':
            $arrpoints = array($x1, $y, $x2, $y, $x, $y2);
            $this->imageFilledPolygon($this->img, $arrpoints, 3, (int)$color);
            break;
        case 'trianglemid':
        ...
        case 'bowtie':
            ...
            break;
        case 'target':
            ImageFilledRectangle($this->img, (int)$x1, (int)$y1, (int)$x, (int)$y, (int)$color);
            ImageFilledRectangle($this->img, (int)$x, (int)$y, (int)$x2, (int)$y2, (int)$color);
            ImageRectangle($this->img, (int)$x1, (int)$y1, (int)$x2, (int)$y2, (int)$color);
            break;
        case 'box':
            ImageRectangle($this->img, (int)$x1, (int)$y1, (int)$x2, (int)$y2, (int)$color);
            break;
        case 'home': /* As in: "home plate" (baseball), also looks sort of like a house. */
            ...
            $this->imageFilledPolygon($this->img, $arrpoints, 5, (int)$color);
            break;
        case 'up':
            ImagePolygon($this->img, array($x, $y1, $x2, $y2, $x1, $y2), 3, (int)$color);
            break;
        case 'down':
            ImagePolygon($this->img, array($x, $y2, $x1, $y1, $x2, $y1), 3, (int)$color);
            break;
        default: /* Also 'rect' */
            ImageFilledRectangle($this->img, (int)$x1, (int)$y1, (int)$x2, (int)$y2, (int)$color);
            break;
        }
        return TRUE;
    }
     ...
     */
     function DrawAreaFillBorders($n_columns, $xd, $yd, $stacked, $stepped)
     {
        for ($col = 1; $col < $n_columns; $col++) {
            ...
            $x = $xd[0];
            $y = $yd[0][$this_col];
            ImageLine($this->img, (int)$x, (int)$yd[0][$other_col], (int)$x, (int)$y, (int)$color);
            ...
            for ($row = 1; $row < $this->num_data_rows; $row++) {
                ...
                $y = $yd[$row][$this_col];
                if ($stepped) {
                    ImageLine($this->img, (int)$prev_x, (int)$prev_y, (int)$x, (int)$prev_y, (int)$color);
                    ImageLine($this->img, (int)$x, (int)$prev_y, (int)$x, (int)$y, (int)$color);
                } else {
                    ImageLine($this->img, (int)$prev_x, (int)$prev_y, (int)$x, (int)$y, (int)$color);
                }
            }

            // Right edge:
            ImageLine($this->img, (int)$x, (int)$y, (int)$x, (int)$yd[$this->num_data_rows - 1][$other_col], (int)$color);
        }
        return TRUE;
     }
...
/////////////////////////////////////////////
            ...
            for ($j = 0; $j < $num_slices; $j++) {
                ...
                if ($arc_start_angle > $arc_end_angle) { // Skip segments with angle < 1 degree
                    ...
                    if ($labels_outside) {   // Labels are outside the pie chart
                        [$width, $height] = $this->SizeText('generic', 0, $labels[$j]);
            ...
            for ($idx = 0; $rec < $this->num_recs[$row]; $rec++, $idx++) {
                if (is_numeric($dv = $this->data[$row][$rec])) {          // Allow for missing data
                    ...
                    if ($this->datatype_swapped_xy) {
                        ...
                        ImageLine($this->img, (int)$this->y_axis_x_pixels, (int)$y_now_pixels,
                                              $this->xtr($dv), (int)$y_now_pixels, (int)$data_color);
                    } else {
                        // Draw a line from user defined x axis position up (or down) to ytr($dv)
                        ImageLine($this->img, (int)$x_now_pixels, (int)$this->x_axis_y_pixels,
                                  (int)$x_now_pixels, $this->ytr($dv), (int)$data_color);
                   }
                }
            }
        }
    ...
    protected function DrawArea($do_stacked = FALSE)
    {
        ...
        for ($col = 1; $col < $n_columns; $col++) { // 1 extra for X axis artificial column
            ...
            // Draw it:
            $this->imageFilledPolygon($this->img, $pts, (int)$n_rows * 2, (int)$this->ndx_data_colors[$prev_col]);
            ...
            for ($idx = 0; $rec < $this->num_recs[$row]; $idx++) {
                ...
                if (is_numeric($dv = $this->data[$row][$rec++]) && $line_style != 'none') {
                    ...
                    if ($start_lines[$idx]) {
                        ...
                        ImageLine($this->img, (int)$x_now_pixels, (int)$y_now_pixels,
                                  $lastx[$idx] ?? 0, $lasty[$idx] ?? 0, $style);
                    }
            ...
            for ($idx = 0; $record < $this->num_recs[$row]; $record++, $idx++) {
                if (is_numeric($y_now = $this->data[$row][$record])) {         // Allow for missing Y data
                    ...
                    if ($start_lines[$idx]) {
                        ...
                        // Draw the step:
                        $y_mid = $lasty[$idx] ?? 0;
                        ImageLine($this->img, (int)($lastx[$idx] ?? 0), (int)$y_mid, (int)$x_now_pixels, (int)$y_mid, (int)$style);
                        ImageLine($this->img, (int)$x_now_pixels, (int)$y_mid, (int)$x_now_pixels, (int)$y_now_pixels, (int)$style);
                    }
    ...
    protected function DrawSquaredArea($do_stacked = FALSE)
    {
        ...
        for ($col = 1; $col < $n_columns; $col++) { // 1 extra for X axis artificial column
            ...
            $this->imageFilledPolygon($this->img, $pts, 4 * (int)$n_rows - 2, (int)$this->ndx_data_colors[$prev_col]);
            $prev_col = $col;
        }
    ...
    protected function DrawOHLC($draw_candles, $always_fill = FALSE)
    {
        ...
        [$body_thickness, $wick_thickness] = $this->line_widths;
        ...
        for ($row = 0; $row < $this->num_data_rows; $row++) {
            ...
            if ($draw_candles) {
                ...
                if ($yh_pixels < $yb1_pixels) {
                    imageline($this->img, (int)$x_now_pixels, (int)$yb1_pixels, (int)$x_now_pixels, (int)$yh_pixels, (int)$ext_color);
                }
                if ($yl_pixels > $yb2_pixels) {
                    imageline($this->img, (int)$x_now_pixels, (int)$yb2_pixels, (int)$x_now_pixels, (int)$yl_pixels, (int)$ext_color);
                }
            } else {
                // Basic OHLC
                imageline($this->img, (int)$x_now_pixels, (int)$yl_pixels, (int)$x_now_pixels, (int)$yh_pixels, (int)$body_color);
                imagesetthickness($this->img, (int)$wick_thickness);
                imageline($this->img, (int)$x_left, (int)$yo_pixels, (int)$x_now_pixels, (int)$yo_pixels, (int)$ext_color);
                imageline($this->img, (int)$x_right, (int)$yc_pixels, (int)$x_now_pixels, (int)$yc_pixels, (int)$ext_color);
            }
            imagesetthickness($this->img, 1);
        ...
        for ($row = 0; $row < $this->num_data_rows; $row++) {
            ...
            for ($idx = 0; $rec < $this->num_recs[$row]; $rec += 2, $idx++) {
                ...
                if (is_numeric($y_now = $this->data[$row][$rec])) {      //Allow for missing Y data
                    ...
                    // Draw the bubble:
                    ImageFilledEllipse($this->img, (int)$x, (int)$y, (int)$size, (int)$size, (int)$data_color);
    ...
    protected function DrawBoxes()
    {
        ...
        [$box_thickness, $belt_thickness, $whisker_thickness] = $this->line_widths;
        ...
        for ($row = 0; $row < $this->num_data_rows; $row++) {
            ...
            if (isset($yd[0]) && $yd[0] > $yd[1]) {   // Note device Y coordinates are inverted (*-1)
                ...
                imageline($this->img, (int)$xd, (int)$yd[0], (int)$xd, (int)$yd[1], (int)$whisker_style);
                imageline($this->img, (int)$xd - (int)$width2, (int)$yd[0], (int)$xd + (int)$width2, (int)$yd[0], (int)$whisker_color);
            }

            // Draw the upper whisker and T
            if (isset($yd[4]) && $yd[3] > $yd[4]) {   // Meaning: Yworld[3] < Yworld[4]
                imagesetthickness($this->img, (int)$whisker_thickness);
                imageline($this->img, (int)$xd, (int)$yd[3], (int)$xd, (int)$yd[4], (int)$whisker_style);
                imageline($this->img, (int)$xd - (int)$width2, (int)$yd[4], (int)$xd + (int)$width2, (int)$yd[4], (int)$whisker_color);
            }
            ...
            if (isset($yd[2])) {
                ...
                imageline($this->img, (int)$x_left, (int)$yd[2], (int)$x_right, (int)$yd[2], (int)$belt_color);
             }

            // Draw the box
            imagesetthickness($this->img, (int)$box_thickness);
            imagerectangle($this->img, (int)$x_left, (int)$yd[3], (int)$x_right, (int)$yd[1], (int)$box_color);
            imagesetthickness($this->img, 1);
...
/////////////////////////////////////////////
     ...
     * @return bool
     */
    private function imageFilledPolygon($image, array $points, int $num_points, int $color): bool
    {
