								...
								<td><table border="0" width="100%" cellspacing="0" cellpadding="0" data-gx-widget="checkbox">
										<tr>
											<td valign="top"><table class="gx-modules-table left-table" border="0" width="100%" cellspacing="0" cellpadding="2">
													...
													<?php
													$zones_query_raw = "select z.zone_id, c.countries_id, c.countries_name, c.countries_iso_code_2, z.zone_name, z.zone_code, z.zone_country_id from " . TABLE_ZONES . " z, " . TABLE_COUNTRIES . " c where z.zone_country_id = c.countries_id order by c.countries_name, z.zone_name";
													...
													while ($zones = xtc_db_fetch_array($zones_query)) {
													  if ((empty($_GET['cID']) || $_GET['cID'] == $zones['zone_id']) && empty($cInfo) && substr($_GET['action'] ?? '', 0, 3) !== 'new') {
												  ...
												  ?>
														<td class="dataTableContent"><span class="flag-icon flag-icon-<?= strtolower($zones['countries_iso_code_2']) ?>"></span> <?php echo htmlspecialchars($zones['countries_name'], ENT_QUOTES); ?></td>
