<?php
/* --------------------------------------------------------------
   customers_status.php 2017-05-15
   Gambio GmbH
   http://www.gambio.de
  ...
  switch ($_GET['action']) {
    ...
    case 'save':
      ...
      for ($i=0; $i<sizeof($languages); $i++) {
        ...
        $customers_fsk18_purchasable = $_POST['customers_fsk18_purchasable'];
        ...
        $customers_base_status = xtc_db_prepare_input($_POST['customers_base_status']);
        ...
        $sql_data_array = array(
          ...
          'customers_fsk18_purchasable' => xtc_db_prepare_input($customers_fsk18_purchasable),
        ...
        $products_query = xtc_db_query("select price_id, products_id, quantity, personal_offer from personal_offers_by_customers_status_" . xtc_db_input($customers_base_status));
        ...
        $product_data_array = array(
         ...
         xtc_db_perform('personal_offers_by_customers_status_' . $customers_status_id, $product_data_array);
          ...
          $insert_sql_data = array('customers_status_id' => (int)$customers_status_id, 'language_id' => (int)$language_id);
      ...
      if ($_POST['default'] == 'on') {
        xtc_db_query("update `gx_configurations` set `value` = '" . xtc_db_input($customers_status_id) . "' where `key` = 'DEFAULT_CUSTOMERS_STATUS_ID'");
      }
    ...
    case 'deleteconfirm':
      ...
      $customers_status_query = xtc_db_query("select `value` from `gx_configurations` where `key` = 'configuration/DEFAULT_CUSTOMERS_STATUS_ID'");
      $customers_status = xtc_db_fetch_array($customers_status_query);
      if ($customers_status['value'] == $cID) {
        xtc_db_query("update `gx_configurations` set `value` = '' where `key` = 'configuration/DEFAULT_CUSTOMERS_STATUS_ID'");
      }
...
<?php
  ...
  $customers_fsk18_purchasable_array = array(array('id' => '0', 'text' => ENTRY_NO), array('id' => '1', 'text' => ENTRY_YES));
  ...
  while ($customers_status = xtc_db_fetch_array($customers_status_query)) {
    ...
    if ($customers_status['customers_status_show_price'] == '1') {
      ...
      echo '<td class="dataTableContent" align="left">';echo SHOW_NO_PRICE;
    }
    echo '</td>';
    ...
    echo '</td>';
    echo '<td class="dataTableContent" align="center" style="white-space:normal">' . str_replace(',', ', ', htmlspecialchars($customers_status['customers_status_payment_unallowed'])) . '&nbsp;</td>';
    echo '<td class="dataTableContent" align="center" style="white-space:normal">' . str_replace(',', ', ', htmlspecialchars($customers_status['customers_status_shipping_unallowed'])) . '&nbsp;</td>';
    echo "\n";
?>
...
<div class="hidden">
  ...
  switch ($_GET['action']) {
    case 'new':
      ...
      $contents[] = array('text' => '<span class="options-title">' . TEXT_INFO_CUSTOMERS_FSK18_INTRO . '</span>' . ENTRY_CUSTOMERS_FSK18 . ' ' . xtc_draw_pull_down_menu('customers_fsk18_purchasable', $customers_fsk18_array, $cInfo->customers_fsk18, 'data-convert-checkbox="true" data-new-class="pull-right"'));
    ...
    case 'edit':
      ...
      if (DEFAULT_CUSTOMERS_STATUS_ID != $cInfo->customers_status_id && $cInfo->customers_status_id !== '1') $contents[] = array('text' => '<span class="options-title">' . TEXT_SET_DEFAULT . '</span>' . GM_DO_NOT_FOR_GUESTS . '<br /><br />' . xtc_draw_checkbox_field('default')); // BOF GM_MOD
    ...
    default:
      if (is_object($cInfo)) {
        ...
        $contents[] = array('text' => '<br />' . TEXT_INFO_CUSTOMERS_STATUS_PAYMENT_UNALLOWED_INTRO . '<br />' . ENTRY_CUSTOMERS_STATUS_PAYMENT_UNALLOWED . ':<b> ' . htmlspecialchars($cInfo->customers_status_payment_unallowed).'</b>');
        $contents[] = array('text' => '<br />' . TEXT_INFO_CUSTOMERS_STATUS_SHIPPING_UNALLOWED_INTRO . '<br />' . ENTRY_CUSTOMERS_STATUS_SHIPPING_UNALLOWED . ':<b> ' . htmlspecialchars($cInfo->customers_status_shipping_unallowed).'</b>');
