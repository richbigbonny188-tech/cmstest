...
//constants
 ...
 if ($_REQUEST['login'] ?? false){
    if (($_REQUEST['pwd'] ?? false)!=$ACCESS_PWD){
 ...
 if (($_REQUEST['refresh'] ?? false) && ($DB['db'] ?? false) && preg_match('/^show/',$SQLq) ) $SQLq=$SHOW_T;

 if (db_connect('nodie')){
    $time_start=microtime_float();

    if ($_REQUEST['phpinfo'] ?? false){
       ob_start();phpinfo();$html=ob_get_clean();preg_match("/<body[^>]*>(.*?)<\/body>/is",$html,$m);
       $sqldr='<div class="pi">'.$m[1].'</div>';
    }else{
     if ($DB['db']){
      if ($_REQUEST['shex'] ?? false){
       print_export();
      }elseif ($_REQUEST['doex'] ?? false){
       check_xss();do_export();
      }elseif ($_REQUEST['shim'] ?? false){
       print_import();
      }elseif ($_REQUEST['doim'] ?? false){
       check_xss();do_import();
      }elseif ($_REQUEST['dosht'] ?? false){
       check_xss();do_sht();
      }elseif (!($_REQUEST['refresh'] ?? false) || preg_match('/^select|show|explain|desc/i',$SQLq) ){
       if ($SQLq)check_xss();
       do_sql($SQLq);#perform non-select SQL only if not refresh (to avoid dangerous delete/drop)
      }
     }else{
        if ( $_REQUEST['refresh'] ?? false ){
           check_xss();do_sql($SHOW_D);
        }elseif ($_REQUEST['crdb'] ?? false){
...
function display_select($sth,$q){
 ...
 if ($is_sht || $is_shd) {$w='wa';
   ...
   $sqldr.="<div class='dot'>
 MySQL Server:
 &#183; <a href='$url&page_token=" . $t_page_token . "&q=".b64e("show variables")."'>Show Configuration Variables</a>
 &#183; <a href='$url&page_token=" . $t_page_token . "&q=".b64e("show status")."'>Show Statistics</a>
 &#183; <a href='$url&page_token=" . $t_page_token . "&q=".b64e("show processlist")."'>Show Processlist</a> ";
   ...
   $sqldr.="<br>";
   if ($is_sht) $sqldr.="Database: &#183; <a href='$url&page_token=" . $t_page_token . "&q=".b64e("show table status")."'>Show Table Status</a>";
   $sqldr.="</div>";
 }
 if ($is_sht){
 ...
 while($row=mysqli_fetch_row($sth)){
   $sqldr.="<tr class='".$rc[$swapper=!$swapper]."' onclick='tc(this)'>";
   $v=$row[0];
   if ($is_sht){
     $vq='`'.$v.'`';
     $url='?'.$xurl."&db=$dbn&t=".b64e($v);
     $v="<input type='checkbox' name='cb[]' value=\"$vq\"></td>"
     ."<td><a href=\"$url&page_token=" . $t_page_token . "&q=".b64e("select * from $vq")."\">$v</a></td>"
     ."<td>".$row[1]."</td>"
     ."<td align='right'>".$row[4]."</td>"
     ."<td align='right'>".$row[6]."</td>"
     ."<td align='right'>".$row[8]."</td>"
     ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=".b64e("show create table $vq")."\">sct</a></td>"
     ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=".b64e("explain $vq")."\">exp</a></td>"
     ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=".b64e("show index from $vq")."\">ind</a></td>"
     ."<td>&#183;<a href=\"$url&shex=1&rt=$vq\">export</a></td>"
     ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=".b64e("drop table $vq")."\" onclick='return ays()'>dr</a></td>"
     ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=".b64e("truncate table $vq")."\" onclick='return ays()'>tr</a></td>"
     ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=".b64e("optimize table $vq")."\" onclick='return ays()'>opt</a></td>"
     ."<td>&#183;<a href=\"$url&page_token=" . $t_page_token . "&q=".b64e("repair table $vq")."\" onclick='return ays()'>rpr</a></td>"
     ."<td>".hs($row[$fields_num-1]);
     $sqldr.="<td>$v</td>";
   }elseif ($is_shd){
     $url='?'.$xurl."&db=$v";
     $v="<a href=\"$url&page_token=" . $t_page_token . "&q=".b64e("SHOW TABLE STATUS")."\">$v</a></td>"
     ."<td><a href=\"$url&page_token=" . $t_page_token . "&q=".b64e("show create database `$v`")."\">scd</a></td>"
     ."<td><a href=\"$url&page_token=" . $t_page_token . "&q=".b64e("show table status")."\">status</a></td>"
     ."<td><a href=\"$url&page_token=" . $t_page_token . "&q=".b64e("show triggers")."\">trig</a>";
     $sqldr.="<td>$v</td>";
   }else{
     for($i=0;$i<$fields_num;$i++){
      $v=$row[$i];
...
body{font-family:Roboto,Arial,sans-serif;font-size:80%;padding:0;margin:0;color:#212121;}
div{padding:6px}
pre{font-size:125%}
textarea{width:100%}
...
.ft a{color:#002337}
.inv{background-color:#002337;color:#FFF}
.inv a{color:#FFF}
table{border-collapse:collapse}
table.res{width:100%}
table.wa{width:auto}
table.res th,table.res td{padding:6px;border:1px solid #fff;vertical-align:top}
table.restr{vertical-align:top}
tr.e{background-color:#CCC}
tr.o{background-color:#EEE}
tr.e:hover, tr.o:hover{background-color:#D7ECFD}
tr.h{background-color:#002337;color:#fff}
tr.s{background-color:#D7ECFD}
...
.frm input {line-height: 25px;margin-bottom: 12px};
.dot{border-bottom:1px dotted #000}
.ajax{text-decoration:none;border-bottom: 1px dashed}
.qnav{width:30px;margin: 0 6px 6px 0}
.sbtn{width:100px;margin-bottom: 6px}
input[value="Go"]{background:#002337 !important; color:#fff !important; border-color: #002337 !important;border-width: 1px}
.clear{clear:both;height:0;display:block}
input[type="button"],input[type="submit"]{box-sizing:border-box;padding:6px 12px;border-radius:2px;background-image:linear-gradient(#f5f5f5, #ebebeb);border:1px solid #B6B6B6;color:#737373;border-width: 1px;cursor: pointer;outline: none;border-color:#dedede #dedede #cfcfcf}
...
function qtpl(s){
 $('qraw').value=s.replace(/%T/g,'`<?php echo ($_REQUEST['t'] ?? false) ? b64d($_REQUEST['t']) : 'tablename'?>`');
}
<?php if($is_sht){?>
function chkall(cab){
...
<?php if ($_SESSION['is_logged'] && $dbh){ ?>
 | <a href="?<?php eo($xurl)?>&page_token=<?php echo $t_page_token; ?>&q=<?=b64e("show databases");?>">Databases</a>: <select name="db" onChange="frefresh()"><option value='*'> - select/refresh -</option><option value=''> - show all -</option>
<?php echo get_db_select($dbn)?></select>
<?php if($dbn){ $z=" &#183; <a href='".hs($self."?$xurl&db=$dbn"); ?>
<?php echo $z.'&q='.b64e($SHOW_T)?>&page_token=<?php echo $t_page_token; ?>'>show tables</a>
...
<div class="dot" style="padding:6px 0px">
...
function get_db_select($sel=''){
 global $DB,$SHOW_D;
 if (is_array($_SESSION['sql_sd'] ?? null) && ($_REQUEST['db'] ?? null)!='*'){//check cache
    $arr=$_SESSION['sql_sd'];
 }else{
...
<input type="button" value=" Cancel " onclick="window.location='<?php eo($self.'?'.$xurl.'&db='.$DB['db'])?>&page_token=<?php echo $t_page_token; ?>'">
...
function get_open_char($str, $pos){
 ...
 return array($ochar ?? null, $opos ?? null);
}
