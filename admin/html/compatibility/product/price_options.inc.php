<?php
/* --------------------------------------------------------------
   price_options.inc.php 2021-07-22
   Gambio GmbH
   http://www.gambio.de
...
$configurationStorage = new ConfigurationStorage('gm_configuration');
$default_tax_class_id = $configurationStorage->get('DEFAULT_TAX_CLASS_ID');
?>
...
<div class="span12">
	<div class="span6 control-group grid price-options">
		...
		<div class="span4">
            ...
            $pInfo_products_price = $pInfo->products_price ?? 0;
            $pInfo_products_tax_class_id = $pInfo->products_tax_class_id ?? 0;
			$price = PRICE_IS_BRUTTO == 'true' ? $xtPrice->xtcFormat($pInfo_products_price * ((100
			                                                                          + xtc_get_tax_rate($pInfo_products_tax_class_id))
			                                                                         / 100),
			                                               false) : xtc_round($pInfo_products_price,
			                                                                            PRICE_PRECISION);
		...
		<div class="span4">
			...
			if(PRICE_IS_BRUTTO == 'true')
			{
				...
				<span class="info">
				<?php
				echo TEXT_NETTO . $xtPrice->xtcFormat($pInfo_products_price, false);
				?>
				</span>
				<?php
			}
...
{
	if(!empty($group_data[$col]['STATUS_NAME']))
	{
		?>
					...
					<div class="price_option grid remove-margin remove-padding">
						<div class="span6 remove-padding">
							...
							$price = PRICE_IS_BRUTTO
							         == 'true' ? $xtPrice->xtcFormat(get_group_price($group_data[$col]['STATUS_ID'],
							                                               $pInfo->products_id ?? null) * ((100
							                                                                        + xtc_get_tax_rate($pInfo->products_tax_class_id ?? null))
							                                                                       / 100),
                                                                     false) : xtc_round(get_group_price($group_data[$col]['STATUS_ID'],
							                                                                            $pInfo->products_id ?? null),
							                                                            PRICE_PRECISION);
						...
						<div class="span6 remove-padding">
							...
							<?php
							if(PRICE_IS_BRUTTO == 'true'
								&& get_group_price($group_data[$col]['STATUS_ID'], $pInfo->products_id ?? null) != '0'
							)
							{
								?>
								<span class="info">
						...
						<div class="old_personal_offers">
							...
							$result = xtc_db_query("SELECT
							                       ...
							                       . $group_data[$col]['STATUS_ID'] . "
									                                     WHERE
									                                         products_id = '" . ($pInfo->products_id ?? 0) . "' AND quantity != 1
							...
							while($row = xtc_db_fetch_array($result))
							{
								if(PRICE_IS_BRUTTO == 'true')
								{
									...
									$price = xtc_round($row['personal_offer'] * ((100 + $tax) / 100), $xtPrice->currencies[$xtPrice->actualCurr]['decimal_places']);
								}
								else
								{
...
<div class="span12">
	<div class="span6 control-group grid price-options">
		...
		<div class="span4">
			<?php
			echo xtc_draw_input_field('products_discount_allowed', $pInfo->products_discount_allowed ?? null);
			?>
		</div>
	</div>
</div>
...
<div class="span12">
	<div class="span6 control-group grid price-options">
		...
		<div class="span4">
			...
			echo xtc_draw_pull_down_menu('products_tax_class_id', $tax_class_array, (empty($_GET['pID'])) ? $default_tax_class_id : $pInfo->products_tax_class_id);
			?>
		</div>
	</div>
</div>