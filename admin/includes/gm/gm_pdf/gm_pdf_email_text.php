<?php
/* --------------------------------------------------------------
   gm_pdf_email_text.php 2016-10-05
   Gambio GmbH
   http://www.gambio.de
...
<div class="gx-compatibility-table">
	...
	<div class="pull-right">
		<?php
			echo gm_get_language_link('gm_pdf_action.php', 'gm_pdf_content', 'gm_box_submenu_content', $lang_id);
		?>
	</div>
	<br />
		...
		<table border="0" width="100%" cellspacing="0" cellpadding="2" id="gm_table" class="remove-margin">
			...
			<?php foreach($gm_values as $key => $value) {?>
				...
				<tr class="dataTableRow">
					<td valign="top" align="left" class="main dataTableContent configuration-label" style="width: 25%;">
					...
					<td valign="top" align="left" class="main dataTableContent" style="width: 50%;">
						<input type="text" onClick="gm_fadeout_boxes('gm_status');" style="width:600px;" id="<?php echo $key; ?>" value="<?php echo htmlspecialchars_wrapper($value); ?>" />
						<?php
						if($key === 'GM_PDF_EMAIL_SUBJECT') {
							echo '<br/ >' . GM_PDF_TITLE_EMAIL_SUBJECT_INFO;
						} elseif ($key === 'GM_PDF_INVOICES_EMAIL_SUBJECT') {
							echo '<br/ >' . GM_PDF_TITLE_INVOICES_EMAIL_SUBJECT_INFO .  '<br /><br />';
						}
						?>
					</td>
				</tr>
