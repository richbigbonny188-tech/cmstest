<?php
/* --------------------------------------------------------------
   $Id: html_graphs.php 950 2022-08-09 16:45:21Z mz $   
...
defined( '_VALID_XTC' ) or die( 'Direct Access to this location is not allowed.' );
  ...
  function xtc_banner_graph_monthly($banner_id) {
    ...
    $year = (($_GET['year']) ? $_GET['year'] : date('Y'));
      ...
      for ($i = 1; $i < 13; $i++) {
          if (extension_loaded('intl')) {
              $names[] = DateFormatter::formatAsFullMonth(DateTime::createFromFormat('n', $i),
                                                          new LanguageCode(new StringType($_SESSION['language_code'])));
          } else {
              $names[] = date('F', mktime(0, 0, 0, $i));
          }
    ...
    while ($banner_stats = xtc_db_fetch_array($banner_stats_query)) {
        if (extension_loaded('intl')) {
            $names[($banner_stats['banner_month']
                    - 1)] = DateFormatter::formatAsFullMonth(DateTime::createFromFormat('n',
                                                                                        $banner_stats['banner_month']),
                                                             new LanguageCode(new StringType($_SESSION['language_code'])));
        } else {
            $names[($banner_stats['banner_month'] - 1)] = date('F', mktime(0, 0, 0, $banner_stats['banner_month']));
        }
  ...
  function xtc_banner_graph_daily($banner_id) {
    ...
    $graph_vals = @array('vlabel'=>TEXT_BANNERS_DATA,
                        'hlabel'=>sprintf(TEXT_BANNERS_DAILY_STATISTICS, $banner['banners_title'], $monthName, $year),
                        'type'=>'3',
                        'cellpadding'=>'',
                        'cellspacing'=>'1',
                        'border'=>'',
