...
use Gambio\StyleEdit\Core\Services\Validator;
...
function check_stock($products_id)
{
    ...
    $stock_warn = (float)$stock_values['products_quantity'];
    $stock_flag = '';
	...
	if(ATTRIBUTE_STOCK_CHECK == 'true') {
        $attribute_stock_query =
            xtc_db_query("SELECT DISTINCT attributes_stock, options_values_id FROM " . TABLE_PRODUCTS_ATTRIBUTES
                         . " where products_id = '" . $products_id . "' and attributes_stock <= 0");
        while ($attribute_stock_values = xtc_db_fetch_array($attribute_stock_query)) {
            $stock_flag            = 'true';
            $which_attribute_query =
                xtc_db_query("SELECT products_options_values_name FROM " . TABLE_PRODUCTS_OPTIONS_VALUES
	...
	{
		return (double)$stock_values['products_quantity'];
	}
}
...
// TODO merge with function in inc folder
...
function xtc_get_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false)
{
	...
	if($include_itself)
	{
		$category_query = xtc_db_query("select cd.categories_name from " . TABLE_CATEGORIES_DESCRIPTION . " cd where cd.language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "' and cd.categories_id = '" . (int)$parent_id . "'");
		$category = xtc_db_fetch_array($category_query);
		$category_tree_array[] = array('id' => $parent_id, 'text' => htmlspecialchars_wrapper($category['categories_name']));
	}

	$categories_query = xtc_db_query("select c.categories_id, cd.categories_name, c.parent_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = cd.categories_id and cd.language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "' and c.parent_id = '" . (int)$parent_id . "' order by c.sort_order, cd.categories_name");
	while($categories = xtc_db_fetch_array($categories_query))
	{
...
function xtc_draw_products_pull_down($name, $parameters = '', $exclude = '')
{
	...
	$select_string .= '>';
	$products_query = xtc_db_query("select p.products_id, pd.products_name,p.products_tax_class_id, p.products_price from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and pd.language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "' order by products_name");
	while($products = xtc_db_fetch_array($products_query))
	{
		if(!xtc_in_array($products['products_id'], $exclude))
		{
			...
			$select_string .= '<option value="' . $products['products_id'] . '">' . htmlspecialchars($products['products_name']) . ' (' . trim($xtPrice->xtcFormat(xtc_round($products['products_price'], PRICE_PRECISION), true)) . ')</option>';
		}
	}
...
function xtc_options_name($options_id)
{
	...
	$options = xtc_db_query("select products_options_name from " . TABLE_PRODUCTS_OPTIONS . " where products_options_id = '" . (int)$options_id . "' and language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "'");
...
function xtc_values_name($values_id)
{
	...
	$values = xtc_db_query("select products_options_values_name from " . TABLE_PRODUCTS_OPTIONS_VALUES . " where products_options_values_id = '" . (int)$values_id . "' and language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "'");
...
function xtc_info_image($image, $alt, $width = '', $height = '', $params = '')
{
	...
	{
		$image = defined('TEXT_IMAGE_NONEXISTENT') ? TEXT_IMAGE_NONEXISTENT : 'IMAGE DOES NOT EXIST';
	}
...
// TODO merge with function from inc folder
...
require_once DIR_FS_INC . 'xtc_browser_detect.inc.php';
...
function xtc_get_categories_name($category_id, $language_id)
{
	...
	$category = xtc_db_num_rows($category_query) ? xtc_db_fetch_array($category_query) : ['categories_name' => ''];
...
function xtc_get_orders_status()
{
	...
	$orders_status_query = xtc_db_query("select orders_status_id, orders_status_name from " . TABLE_ORDERS_STATUS . " where language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "' order by orders_status_id");
...
function xtc_get_cross_sell_groups()
{
	...
	$cross_sell_query = xtc_db_query("select products_xsell_grp_name_id, groupname from " . TABLE_PRODUCTS_XSELL_GROUPS . " where language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "' order by products_xsell_grp_name_id");
...
function xtc_get_shipping_status()
{
	...
	$shipping_status_query = xtc_db_query("select shipping_status_id, shipping_status_name from " . TABLE_SHIPPING_STATUS . " where language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "' order by number_of_days, shipping_status_id");
...
function xtc_get_system_information()
{
	...
	[$system, $host, $kernel] = preg_split('/[\s,]+/', php_uname(), 5);

	return array('date' => xtc_datetime_short(date('Y-m-d H:i:s')), 'system' => $system, 'kernel' => $kernel, 'host' => $host, 'ip' => gethostbyname($host), 'http_server' => $_SERVER['SERVER_SOFTWARE'], 'php' => PHP_VERSION, 'zend' => (function_exists('zend_version') ? zend_version() : ''), 'db_server' => DB_SERVER, 'db_ip' => gethostbyname(DB_SERVER), 'db_version' => 'MySQL ' . (function_exists('mysqli_get_server_info') ? ((is_null($___mysqli_res = mysqli_get_server_info($GLOBALS["___mysqli_ston"]))) ? false : $___mysqli_res) : ''), 'db_date' => xtc_datetime_short($db['datetime']));
}
...
function xtc_generate_category_path($id, $from = 'category', $categories_array = '', $index = 0)
{
	...
	if($from == 'product')
	{
		...
		while($categories = xtc_db_fetch_array($categories_query))
		{
			...
			{
				$category_query = xtc_db_query("select cd.categories_name, c.parent_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = '" . (int)$categories['categories_id'] . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "'");
				$category = xtc_db_fetch_array($category_query);
	...
	{
		$category_query = xtc_db_query("select cd.categories_name, c.parent_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = '" . (int)$id . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "'");
		$category = xtc_db_fetch_array($category_query);
		$categories_array[$index][] = array('id' => $id, 'text' => $category['categories_name'] ?? null);
		if((xtc_not_null($category['parent_id'] ?? null)) && ($category['parent_id'] ?? null != '0'))
			$categories_array = xtc_generate_category_path($category['parent_id'], 'category', $categories_array, $index);
	}
...
function xtc_output_generated_category_path($id, $from = 'category')
{
	...
	for($i = 0, $n = sizeof($calculated_category_path); $i < $n; $i++)
	{
		...
		{
			$calculated_category_path_string .= htmlspecialchars($calculated_category_path[$i][$j]['text'] ?? '') . '&nbsp;&gt;&nbsp;';
		}
...
function xtc_remove_order($order_id, $restock = false, $canceled = false, $reshipp = false, $reactivateArticle = false)
{
		...
		while($order = xtc_db_fetch_array($order_query))
		{
			if($restock == 'on')
			{
				...
				if($t_combis_array_length > 0)
				{
					...
					$t_use_combis_quantity = (int)$coo_combis_admin_control->get_use_properties_combis_quantity($order['products_id']);
				}
				else
				{
				...
				if($stockService->isChangeProductStock((int)$t_use_combis_quantity, (int)$order['products_properties_combis_id'], $order['products_attributes_filename']))
				{
					xtc_db_query('
				...
				if($t_combis_array_length > 0 && (($t_use_combis_quantity === PropertiesCombisAdminControl::DEFAULT_GLOBAL && STOCK_CHECK == 'true' && ATTRIBUTE_STOCK_CHECK == 'true') || $t_use_combis_quantity === PropertiesCombisAdminControl::COMBI_STOCK))
				{
					xtc_db_query("
                                    UPDATE
                                        products_properties_combis
	...
	if(!$canceled)
	{
		...
		xtc_db_query('DELETE FROM orders_payment_instruction WHERE orders_id = ' . (int)$order_id);

		// BOF GM_MOD GX-Customizer:
		require_once('../gm/modules/gm_gprint_admin_general.php');
		
		$db = StaticGXCoreLoader::getDatabaseQueryBuilder();
		$db->set('order_id', 0)->where('order_id', $order_id)->update('invoices');
		$db->set('order_id', 0)->where('order_id', $order_id)->update('packing_slips');
	}
	
	$deleteHistoryService = DeleteHistoryServiceFactory::writeService();
	$deleteHistoryService->reportDeletion(DeletedId::create($order_id), DeleteHistoryScope::orders());
}
...
/**
 * @param        $values
 * @param        $deactivatedValues
 * @param string $default
 *
 * @return mixed|string
 */
function pull_down_template_sets_with_categories($values, $deactivatedValues, $default = '')
{
    $activeTemplatePullDown = xtc_draw_pull_down_menu('configuration_value', $values, $default);
    
    if (!is_array($deactivatedValues) || !count($deactivatedValues)) {
        return $activeTemplatePullDown;
    }
    
    $selectMarkUp      = '<select name="configuration_value">';
    $selectCloseMarkUp = '</select>';
    $deactivatedValues = xtc_draw_pull_down_menu('configuration_value', $deactivatedValues, $default);
    
    $languageTextManager = MainFactory::create('LanguageTextManager', 'configuration', $_SESSION['languages_id']);
    $optGroupLabel       = $languageTextManager->get_text('CURRENT_THEME_UPDATE_REQUIRED');
    
    $deactivatedValues = str_replace([$selectMarkUp, $selectCloseMarkUp],
                                     ['<optgroup label="' . $optGroupLabel . '" disabled>', '</optgroup>'],
                                     $deactivatedValues);
    
    return str_replace($selectCloseMarkUp, $deactivatedValues . $selectCloseMarkUp, $activeTemplatePullDown);
}

/**
 * This function is used by xtc_cfg_pull_down_template_sets
 *
 * @param string $themePath
 * @return stdClass|null
 */
function get_theme_json_from_theme_path(string $themePath)
{
    $themeJsonPath = $themePath . DIRECTORY_SEPARATOR . 'theme.json';
    
    if (is_dir($themePath) && is_file($themeJsonPath)) {
        
        $json = json_decode(file_get_contents($themeJsonPath), false);
...
function xtc_cfg_pull_down_order_statuses($order_status_id, $key = '')
{
	...
	$statuses_query = xtc_db_query("select orders_status_id, orders_status_name from " . TABLE_ORDERS_STATUS . " where language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "' order by orders_status_name");
...
function xtc_get_customers_statuses()
{
    ...
    $customers_statuses_query = xtc_db_query("select customers_status_id, customers_status_name, customers_status_image, customers_status_discount, customers_status_ot_discount_flag, customers_status_ot_discount, customers_status_public, customers_status_graduated_prices from " . TABLE_CUSTOMERS_STATUS . " where language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "' order by customers_status_id");
...
function xtc_get_customer_status($customers_id)
{
	...
	$customer_status_query = xtc_db_query("select customers_status, member_flag, customers_status_name, customers_status_public, customers_status_image, customers_status_discount, customers_status_ot_discount_flag, customers_status_ot_discount, customers_status_graduated_prices  FROM " . TABLE_CUSTOMERS . " left join " . TABLE_CUSTOMERS_STATUS . " on customers_status = customers_status_id where customers_id='" . (int)$customers_id . "' and language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "'");
...
/**
 * Creates an input field of type number for configuration values.
 ...
 */
function cfg_number_input($name)
{
...
/**
 * Creates an input field of type number for required numeric configuration values.
 *
 * IMPORTANT NOTE: The name attribute gets replaced in admin/configuration.php:839
 *
 * @param string        $name Value of name attribute.
 * @param string|double $step Step amount.
 *
 * @return string Input element of type number.
 */
function cfg_number_required_step_input($step, $name)
{
    return '<input type="number" step="' . $step . '" min="0" name="configuration_value" value="' . $name
           . '" required/>';
}
...
/**
 * Creates an input field of type number for required numeric configuration values with a minimum, maximum and step
 * value.
 *
 * IMPORTANT NOTE: The name attribute gets replaced in admin/configuration.php:839
 *
 * @param string|int    $min  Allowed minimum value.
 * @param string|int    $max  Allowed maximum value.
 * @param string|double $step Step amount.
 * @param string        $name Value of name attribute.
 *
 * @return string Input element of type number.
...
{
    return '<input type="number" step="' . $step . '" min="' . $min . '" max="' . $max
           . '" name="configuration_value" value="' . $name . '" required />';
}
...
function cfg_cod_fee_transfer_charge_form($t_values_string)
{
	$t_installed_shipping_filenames_array = array();
	$t_values_array = array();
	$t_shipping_array = array();
	...
	foreach($t_shipping_array AS $t_module_name => $t_rules)
	{
		$t_html_output .= '<input type="hidden" name="configuration[MODULE_ORDER_TOTAL_COD_FEE_TRANSFER_CHARGE][]" value="' . $t_module_name . '" />';
		...
		$t_html_output .= '<strong>' . $t_shipping_name . '</strong><br /><input type="text" name="configuration[MODULE_ORDER_TOTAL_COD_FEE_TRANSFER_CHARGE][]" value="' . htmlspecialchars_decode($t_rules) . '" /><br /><br />';
	}
