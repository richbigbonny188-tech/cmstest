...
if(@is_writeable(gm_local_install_path() . 'admin/includes/configure.org.php')
    ...
    && @is_writeable(gm_local_install_path() . 'config'))
{
	$db = array();
...
CODE;
		...
		$file_contents = '<?php' . "\n" .
						 ...
						 'define(\'HTTP_SERVER\', \'' . (($_POST['ENABLE_SSL'] == 'true') ? 'https://' . $_POST['HTTP_SERVER'] : 'http://' . $_POST['HTTP_SERVER']) . '\'); // eg, http://localhost - should not be empty for productive servers' . "\n" .
						 'define(\'HTTPS_SERVER\', \'' . 'https://' . $_POST['HTTP_SERVER'] . '\'); // eg, https://localhost - should not be empty for productive servers' . "\n" .
                         ...
                         '//define(\'USE_PCONNECT\', \'' . ((($_POST['USE_PCONNECT'] ?? '') == 'true') ? 'true' : 'false') . '\'); // use persistent connections? deprecated and not supported anymore since GX 4.5' . "\n" .
                         $defineDatabaseFunction . "\n" .
						 'define_database(\'' . trim($_POST['DB_SERVER']) . '\', // eg, localhost - should not be empty for productive servers' . "\n" .
						 '                \'' . trim($_POST['DB_SERVER_USERNAME']) . '\',' . "\n" .
                         '                \'' . trim($_POST['DB_SERVER_PASSWORD']). '\',' . "\n" .
						 '                \'' . trim($_POST['DB_DATABASE']). '\',' . "\n" .
						 '                \'' . ((($_POST['USE_PCONNECT'] ?? '') == 'true') ? 'true' : 'false') . '\'); // use persistent connections? deprecated and not supported anymore since GX 4.5' . "\n" .
						$t_code_block_frontend_end;
		...
		$file_contents = '<?php' . "\n" .
						 ...
						 '// * DIR_FS_* = Filesystem directories (local/physical)' . "\n" .
						 '// * DIR_WS_* = Webserver directories (virtual/URL)' . "\n" .
						 'define(\'HTTP_SERVER\', \'' . (($_POST['ENABLE_SSL'] == 'true') ? 'https://' . $_POST['HTTP_SERVER'] : 'http://' . $_POST['HTTP_SERVER']) . '\'); // eg, http://localhost or - https://localhost should not be empty for productive servers' . "\n" .
						 'define(\'HTTP_CATALOG_SERVER\', \'' . (($_POST['ENABLE_SSL'] == 'true') ? 'https://' . $_POST['HTTP_SERVER'] : 'http://' . $_POST['HTTP_SERVER']) . '\');' . "\n" .
						 'define(\'HTTPS_CATALOG_SERVER\', \'' . 'https://' . $_POST['HTTP_SERVER'] . '\');' . "\n" .
	...
	$envFilePath = gm_local_install_path() . 'config/.env.php';
...
ENV;
	...
	file_put_contents($envFilePath, sprintf($envFileContent, bin2hex(random_bytes(16))));
