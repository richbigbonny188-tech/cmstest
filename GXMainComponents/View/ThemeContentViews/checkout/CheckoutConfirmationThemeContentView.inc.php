<?php
/*--------------------------------------------------------------------
 CheckoutConfirmationThemeContentView.inc.php 2022-07-27
 Gambio GmbH
 http://www.gambio.de
...
class CheckoutConfirmationThemeContentView extends ThemeContentView
{
        ...
        if (empty($t_uninitialized_array)) {
            ...
            for ($i = 0, $n = sizeof($coo_order->products); $i < $n; $i++) {
                ...
                $t_products_item       = [
                    ...
                    'products_image'       => $this->getMainImage($coo_order->products[$i]['id'],
                                                                  $coo_order->products[$i]),
            ...
            if (array_key_exists($t_payment, $GLOBALS) && isset($GLOBALS[$t_payment]) && method_exists($GLOBALS[$t_payment], 'confirm_pre_form')) {
     ...
     * @param product_ORIGIN $product   unused and deprecated
     *
     * @return string
     */
    protected function getMainImage(string $productId, $product): string
    {
        $image = ProductMainImageProvider::getImage($productId);
