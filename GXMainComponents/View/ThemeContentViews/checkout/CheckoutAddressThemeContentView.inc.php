<?php
/* --------------------------------------------------------------
  CheckoutAddressThemeContentView.inc.php 2018-11-23
  Gambio GmbH
  http://www.gambio.de
...
class CheckoutAddressThemeContentView extends ThemeContentView
{
    ...
    public function __construct()
    {
        ...
        $this->unallowedClassesArray   = [];
        $this->unallowedClassesArray[] = 'packstation';
        $this->unallowedClassesArray[] = 'postfiliale';
        $this->unallowedClassesArray[] = 'packstation_2';
        $this->unallowedClassesArray[] = 'postfiliale_2';
        $this->unallowedClassesArray[] = 'parcelshop';
    }
    ...
    public function initPageData()
    {
        if ($this->page_type == 'shipping') {
            $this->set_content_template('checkout_shipping_address.html');
            ...
            $this->set_content_template('checkout_payment_address.html');
    ...
    protected function _addAddress(array $addressDataArray, array &$addressesDataArray)
    {
        ...
        $addressDataArray['address'] = xtc_address_format($formatId, $addressDataArray, true, ' ', ', ');
        $addressesDataArray[]        = $addressDataArray;
    }
    ...
    protected function _isUnallowedAddress($addressClass, $streetAddress)
    {
        if ($this->page_type == 'payment'
            && (in_array((string)$addressClass, $this->unallowedClassesArray)
                || preg_match('/.*(' . implode('|', $this->unallowedClassesArray) . ').*/i', (string)$streetAddress)
                   == 1)) {
            return true;
        }
    ...
    protected function _assignUrls()
    {
        ...
        if ((bool)gm_get_conf('MODULE_CENTER_PARCELSHOPFINDER_INSTALLED') === true && $this->page_type !== 'payment') {
            $this->set_content_data('BUTTON_PARCELSHOPFINDER_URL',
