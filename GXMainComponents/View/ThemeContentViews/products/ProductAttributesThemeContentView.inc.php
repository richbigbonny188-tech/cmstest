<?php
/* --------------------------------------------------------------
  ProductAttributesThemeContentView.inc.php 2021-06-17
  Gambio GmbH
  http://www.gambio.de
 ...
 */
class ProductAttributesThemeContentView extends ThemeContentView
{
    ...
    public function prepare_data()
    {
        ...
        $t_uninitialized_array = $this->get_uninitialized_variables(['coo_product', 'language_id']);
    ...
    protected function get_attribute_options_data(array $p_row)
    {
        ...
        if ($_SESSION['customers_status']['customers_status_show_price'] == '1'
            ...
            $t_attribute_option_data_array['FULL_PRICE'] = $this->coo_xtc_price->xtcFormat($this->get_attribute_option_full_price($p_row,
                                                                                                                                  $t_price),
                                                                                           true);
    ...
    protected function get_attribute_option_image(array $p_row)
    {
        ...
        if (!empty($p_row['gm_filename'])) {
            $t_attribute_option_image = DIR_WS_IMAGES . 'product_images/option_images/' . $p_row['gm_filename'];
        }
    ...
    protected function get_attribute_option_price(array $p_row)
    {
        ...
        if ($p_row['options_values_price'] != '0.00') {
            if ($this->coo_product->data['products_tax_class_id'] != 0) {
                $t_price = $this->coo_xtc_price->xtcFormat($p_row['options_values_price'],
                                                           false,
                                                           $this->coo_product->data['products_tax_class_id']);
            } else {
                $t_price = $this->coo_xtc_price->xtcFormat($p_row['options_values_price'],
                                                           false,
                                                           $this->coo_product->data['products_tax_class_id'],
        ...
        if ($this->coo_xtc_price->xtcCheckDiscount($this->coo_product->data['products_id'])
    ...
    {
        $t_products_price = $this->coo_xtc_price->xtcGetPrice($this->coo_product->data['products_id'],
                                                              $format = false,
                                                              1,
                                                              $this->coo_product->data['products_tax_class_id'],
                                                              $this->coo_product->data['products_price']);
     ...
     */
    public function set_gm_product_option_template($template)
    {
        $this->set_content_template($template);
    }
}
