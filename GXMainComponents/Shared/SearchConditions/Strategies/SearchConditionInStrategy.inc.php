<?php
/* --------------------------------------------------------------
   SearchConditionInStrategy.inc.php 2018-07-02
   Gambio GmbH
   http://www.gambio.de
 ...
 */
class SearchConditionInStrategy implements SearchConditionStrategyInterface
{
    ...
    protected function getValue(array $params)
    {
        ...
        $params[$key] = array_map(function ($element) {
            return addslashes($element);
        },
            $params[$key]);
        
        return '("' . implode('", "', $params[$key]) . '")';
    }
}